...
def make_alignment(
    reference_file, reads_file, num_proc, platform, read_type, out_alignment, args=None
):
    mode = platform + "-" + read_type
    _run_minimap(reference_file, reads_file, num_proc, mode, out_alignment, args)
...
def _run_minimap(
    reference_file, reads_files, num_proc, reads_type, out_file, args=None
):
    ...
    if args is not None and args.directional_reads:
