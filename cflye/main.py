...
import cflye.polishing.alignment as aln
import cflye.polishing.polish as pol
import cflye.polishing.consensus as cons
import cflye.assembly.assemble as asm
import cflye.assembly.repeat_graph as repeat
import cflye.assembly.scaffolder as scf
from cflye.__version__ import __version__
from cflye.__build__ import __build__
import cflye.config.py_cfg as cfg
from cflye.config.configurator import setup_params, ConfigException
from cflye.utils.bytes2human import human2bytes, bytes2human
from cflye.utils.sam_parser import AlignmentException
import cflye.utils.fasta_parser as fp

# import cflye.short_plasmids.plasmids as plas
# import cflye.trestle.trestle as tres
# import cflye.trestle.graph_resolver as tres_graph
from cflye.repeat_graph.repeat_graph import RepeatGraph
from cflye.six.moves import range
...
class Job(object):
    ...
    def load(self, save_file):
        with open(save_file, "r") as f:
            data = json.load(f)
            if (
                not "pipeline_version" in data
                or data["pipeline_version"] != cfg.vals["pipeline_version"]
            ):
...
class JobAssembly(Job):
    def __init__(self, args, work_dir, log_file):
        ...
        self.assembly_filename = os.path.join(self.assembly_dir, "draft_assembly.fasta")
...
class JobRepeat(Job):
    def __init__(self, args, work_dir, log_file, disjointigs):
        ...
        self.out_files["repeat_graph"] = os.path.join(
            self.work_dir, "repeat_graph_dump"
        )
        self.out_files["repeat_graph_edges"] = os.path.join(
            self.work_dir, "repeat_graph_edges.fasta"
        )
        self.out_files["reads_alignment"] = os.path.join(
            self.work_dir, "read_alignment_dump"
        )
...
class JobContigger(Job):
    def __init__(
        ...
        self.out_files["contigs"] = os.path.join(self.work_dir, "contigs.fasta")
        self.out_files["assembly_graph"] = os.path.join(self.work_dir, "graph_final.gv")
        self.out_files["edges_sequences"] = os.path.join(
            self.work_dir, "graph_final.fasta"
        )
        self.out_files["gfa_graph"] = os.path.join(self.work_dir, "graph_final.gfa")
        self.out_files["stats"] = os.path.join(self.work_dir, "contigs_stats.txt")
        self.out_files["scaffold_links"] = os.path.join(
            self.work_dir, "scaffolds_links.txt"
        )
    ...
    def run(self):
        ...
        repeat.generate_contigs(
            self.args,
            Job.run_params,
            self.repeat_graph_edges,
            self.work_dir,
            self.log_file,
            self.args.asm_config,
            self.repeat_graph,
            self.reads_alignment,
        )

        if os.path.getsize(self.out_files["contigs"]) == 0:
            raise asm.AssembleException(
                "No contigs were assembled - " "pipeline stopped"
            )
...
class JobFinalize(Job):
    ...
    def run(self):
        ...
        scaffolds = scf.generate_scaffolds(
            self.contigs_file, scf_links, self.out_files["assembly"]
        )
        ...
        logger.debug("--------------------------")
        scf.generate_stats(
            self.repeat_stats, self.polished_stats, scaffolds, self.out_files["stats"]
        )
...
class JobPolishing(Job):
    def __init__(
        self, args, work_dir, log_file, in_contigs, in_graph_edges, in_graph_gfa
    ):
        ...
        self.name = "polishing"
        final_contigs = os.path.join(self.polishing_dir, "filtered_contigs.fasta")
        self.out_files["contigs"] = final_contigs
        self.out_files["stats"] = os.path.join(self.polishing_dir, "filtered_stats.txt")
        self.out_files["polished_gfa"] = os.path.join(
            self.polishing_dir, "polished_edges.gfa"
        )
...
def _create_job_list(args, work_dir, log_file):
    ...
    jobs.append(
        JobContigger(
            args, work_dir, log_file, repeat_graph_edges, repeat_graph, reads_alignment
        )
    )
    ...
    if args.num_iters > 0:
        jobs.append(
            JobPolishing(
                args, work_dir, log_file, raw_contigs, final_graph_edges, gfa_file
            )
        )
...
def _run_polisher_only(args, output_progress=True):
    ...
    for read_file in args.reads:
        ...
        if not any(
            [without_gz.endswith(x) for x in ["fasta", "fa", "fastq", "fq", "bam"]]
        ):
    ...
    if bam_input and args.num_iters > 1:
        raise ResumeException(
            "Bam input only supports single iteration. For multiple iterations, provide fastq instead"
        )
...
def _run(args):
    ...
    Runs the pipeline
    """
    logger.info("Starting cFlye " + _version())
    logger.debug("Cmd: %s", " ".join(sys.argv))
    ...
    if args.stop_after and not args.stop_after in [j.name for j in jobs]:
        raise ResumeException("Stop after: unknown stage '{0}'".format(args.stop_after))
        ...
        for i in range(len(jobs)):
            if jobs[i].name == job_to_resume:
                ...
                if not jobs[i - 1].completed(save_file):
                    raise ResumeException(
                        "Can't resume: stage '{0}' incomplete".format(jobs[i - 1].name)
                    )
                can_resume = True
                break
...
def _usage():
    return (
        "cflye (--pacbio-raw | --pacbio-corr | --pacbio-hifi | --nano-raw |\n"
...
def main():
    def check_int_range(value, min_val, max_val, require_odd=False):
        ...
        if ival < min_val or ival > max_val:
            raise argparse.ArgumentTypeError(
                "value should be in the " "range [{0}, {1}]".format(min_val, max_val)
            )
    ...
    parser.add_argument(
        ...
        metavar="int",
        default=None,
        help="reduced coverage for initial " "disjointig assembly [not set]",
        type=int,
    )
    parser.add_argument(
        "--hifi-error",
    ...
    if args.read_error:
        hifi_str = (
            "assemble_ovlp_divergence={0},repeat_graph_ovlp_divergence={0}".format(
                args.read_error
            )
        )
    ...
    args.log_file = os.path.join(args.out_dir, "cflye.log")
    _enable_logging(args.log_file, args.debug, overwrite=False)

    args.asm_config = os.path.join(
        cfg.vals["pkg_root"], cfg.vals["bin_cfg"][args.read_type]
    )
