...
import cflye.short_plasmids.utils as utils
import cflye.short_plasmids.unmapped_reads as unmapped
import cflye.utils.fasta_parser as fp
from cflye.utils.sam_parser import read_paf, read_paf_grouped
import logging
...
def extract_circular_reads(unmapped_reads_mapping, max_overhang=150):
    ...
    for current_hit in read_paf(unmapped_reads_mapping):
        if is_circular_read(current_hit, max_overhang):
            ...
            if (
                hit is None
                or current_hit.query_mapping_length() > hit.query_mapping_length()
            ):
...
def mapping_segments_without_intersection(circular_pair):
    if (
        not circular_pair[1].query_start
        < circular_pair[1].query_end
        < circular_pair[0].query_start
        < circular_pair[0].query_end
    ):
        return False

    if (
        not circular_pair[0].target_start
        < circular_pair[0].target_end
        < circular_pair[1].target_start
        < circular_pair[1].target_end
    ):
        return False
...
def extract_circular_pairs(unmapped_reads_mapping, max_overhang=300):
    ...
    for hit_group in read_paf_grouped(unmapped_reads_mapping):
        ...
        for hit in hit_group:
            if (
                not has_overlap
                and hit.query_right_overhang() < max_overhang
                and hit.target_left_overhang() < max_overhang
            ):
                has_overlap = True
                circular_pair[0] = hit
                continue
            ...
            if (
                not is_circular
                and hit.query_left_overhang() < max_overhang
                and hit.target_right_overhang() < max_overhang
            ):
                is_circular = True
                circular_pair[1] = hit
        ...
        if (
            has_overlap
            and is_circular
            and mapping_segments_without_intersection(circular_pair)
        ):
...
def extract_unique_plasmids(
    ...
    for hit_group in read_paf_grouped(trimmed_reads_mapping):
        ...
        if (
            query_mapping_rate > mapping_rate_threshold
    ...
    connected_components, n_components = utils.find_connected_components(
        similarity_graph
    )
