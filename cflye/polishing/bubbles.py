...
from cflye.polishing.alignment import shift_gaps, get_uniform_alignments
from cflye.utils.sam_parser import SynchronizedSamReader, SynchonizedChunkManager
from cflye.utils.utils import process_in_parallel, get_median
from cflye.six.moves import zip
...
def _thread_worker(
    ...
    try:
        while True:
            ...
            if len(ctg_aln) == 0:
                continue
            ref_seq = aln_reader.get_region_sequence(
                ctg_region.ctg_id, ctg_region.start, ctg_region.end
            )
            ...
            if mean_cov > coverage_cap:
                mean_cov = aln_reader.get_median_depth(
                    ctg_region.ctg_id, ctg_region.start, ctg_region.end
                )
            ##
...
def make_bubbles(
    alignment_path, contigs_info, contigs_path, err_mode, num_proc, bubbles_out
):
    """
    ...
    chunk_feeder = SynchonizedChunkManager(
        contigs_fasta, manager, chunk_size=CHUNK_SIZE
    )
            ...
            for part_num in range(num_chunks):
                ...
                for b in bubble.branches:
                    ...
                    start = part_num * chunk_len
                        ...
                        (part_num + 1) * chunk_len
                        if part_num != num_chunks - 1
                        else len(b)
                    )
...
def _is_solid_kmer(profile, position, err_mode):
    ...
    for i in range(position, position + SOLID_LEN):
        if profile[i].coverage == 0:
            return False
        local_missmatch = (
            profile[i].num_missmatch + profile[i].num_deletions
        ) / profile[i].coverage
...
def _is_simple_kmer(profile, position):
    ...
    nucl_str = [
        p.nucl
        for p in profile[position - extended_len // 2 : position + extended_len // 2]
    ]

    # single nucleotide homopolymers
    for i in range(
        extended_len // 2 - SIMPLE_LEN // 2, extended_len // 2 + SIMPLE_LEN // 2 - 1
    ):
...
def _get_partition(profile, err_mode):
    ...
    while prof_pos < len(profile) - SOLID_LEN:
        ...
        landmark = all(
            solid_flags[prof_pos : prof_pos + SIMPLE_LEN]
