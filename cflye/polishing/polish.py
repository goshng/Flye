...
from cflye.polishing.alignment import (
    make_alignment,
    get_contigs_info,
    merge_chunks,
    split_into_chunks,
)
from cflye.utils.sam_parser import SynchronizedSamReader
from cflye.polishing.bubbles import make_bubbles
import cflye.utils.fasta_parser as fp
from cflye.utils.utils import which
import cflye.config.py_cfg as cfg
from cflye.six import iteritems
from cflye.six.moves import range


POLISH_BIN = "cflye-modules"
...
def check_binaries():
    if not which(POLISH_BIN):
        raise PolishException("polishing binary was not found. " "Did you run 'make'?")
    try:
...
def polish(
    ...
    subs_matrix = os.path.join(
        cfg.vals["pkg_root"], cfg.vals["err_modes"][read_platform]["subs_matrix"]
    )
    hopo_matrix = os.path.join(
        cfg.vals["pkg_root"], cfg.vals["err_modes"][read_platform]["hopo_matrix"]
    )
    ...
    for i in range(num_iters):
        ...
        bubbles_file = os.path.join(work_dir, "bubbles_{0}.fasta".format(i + 1))
        coverage_stats, mean_aln_error = make_bubbles(
            alignment_file,
            contigs_info,
            prev_assembly,
            read_platform,
        ...
        polished_fasta, polished_lengths, bubble_coverages = _compose_sequence(
            consensus_out
        )
    ...
    with open(stats_file, "w") as f:
        ...
        for ctg_id in contig_lengths:
            ...
            f.write(
                "{0}\t{1}\t{2}\n".format(
                    ctg_id, contig_lengths[ctg_id], coverage_stats[ctg_id]
                )
            )
    ...
    with gzip.open(bed_coverage, "wt") as f:
        ...
        for ctg_id in bubble_coverages:
            ...
            for bub_len, bub_cov in bubble_coverages[ctg_id]:
                f.write(
                    "{0}\t{1}\t{2}\t{3}\n".format(
                        ctg_id, cur_pos, cur_pos + bub_len, bub_cov
                    )
                )
                cur_pos += bub_len
...
def generate_polished_edges(
    ...
    make_alignment(
        polished_contigs, [edges_file], num_threads, platform, read_type, alignment_file
    )
    ...
    with open(os.path.join(work_dir, "polished_edges.gfa"), "w") as gfa_polished, open(
        gfa_file, "r"
    ) as gfa_in:
        for line in gfa_in:
    ...
    logger.debug("%d sequences remained unpolished", len(edges_dict) - updated_seqs)
    os.remove(alignment_file)
...
def filter_by_coverage(args, stats_in, contigs_in, stats_out, contigs_out):
    ...
    if args.read_type == "subasm":
        ...
        coverage_threshold = int(round(mean_coverage / RELATIVE_MIN_COVERAGE))
        coverage_threshold = max(HARD_MIN_COVERAGE, coverage_threshold)
    logger.debug(
        "Mean contig coverage: %d, selected threshold: %d",
        mean_coverage,
        coverage_threshold,
    )
    ...
    logger.debug("Filtered %d contigs of total length %d", filtered_num, filtered_seq)
    ...
    with open(stats_out, "w") as f:
        ...
        for ctg_id in good_fasta:
            f.write(
                "{0}\t{1}\t{2}\n".format(
                    ctg_id, ctg_stats[ctg_id][0], ctg_stats[ctg_id][1]
                )
            )
        ...
        for line in f:
            if header:
                ...
                consensuses[ctg_id].append(
                    (ctg_pos, ctg_sub_pos, coverage, line.strip())
                )
            header = not header
