...
std::vector<FastaRecord> OutputGenerator::generatePathSequences(
    const std::vector<UnbranchingPath> &paths) const {
  ...
  for (auto &contig : paths) {
    ...
    for (size_t i = 0; i < contig.path.size(); ++i) {
      ...
      if (bestSegment->seqLen == 0)
        continue;
      nucSequence +=
          _graph.edgeSequences().getSeq(bestSegment->edgeSeqId).str();
    }
    // contigParts.push_back(contigPath);
    contigSequences.push_back(
        {DnaSequence(nucSequence), contig.name(), FastaRecord::ID_NONE});
  }
...
void OutputGenerator::outputGfa(const std::vector<UnbranchingPath> &paths,
  ...
  for (size_t i = 0; i < paths.size(); ++i) {
    if (!paths[i].id.strand())
      continue;

    // size_t kmerCount = sequences[i].sequence.length() *
    // paths[i].meanCoverage;
  ...
  for (size_t i = 0; i < paths.size(); ++i) {
    for (auto &outEdgeIt : edgeConnections[paths[i].path.back()]) {
      ...
      if (usedPairs.count(std::make_pair(edgeLeft, edgeRight)))
        continue;
...
/*void OutputGenerator::outputGfaCompact(const std::vector<UnbranchingPath>&
paths, const std::string& filename)
{
        auto sequences = this->generatePathSequences(paths);
        std::unordered_map<GraphNode*, std::vector<const UnbranchingPath*>>
leftNodes; std::unordered_map<GraphNode*, std::vector<const UnbranchingPath*>>
rightNodes; for (auto& path : paths)
        {
        ...
        for (size_t i = 0; i < paths.size(); ++i)
        {
                ...
                //size_t kmerCount = sequences[i].sequence.length() *
paths[i].meanCoverage; fprintf(fout, "S\t%s\t%s\tdp:i:%d\n",
paths[i].name().c_str(), sequences[i].sequence.str().c_str(),
(int)paths[i].meanCoverage);
        }
        ...
        for (auto& nodeIt : leftNodes)
        {
                if (rightNodes.count(nodeIt.first))
                {
                        ...
                        {
                                signedNodeIds[nodeIt.first] =
(int)nodeIt.first->nodeId; GraphNode* complNode =
_graph.complementNode(nodeIt.first); if (complNode != nodeIt.first)
                                {
                                        signedNodeIds[complNode] =
-(int)nodeIt.first->nodeId;
                                }

                                std::string nodeId = "node_" +
std::to_string(nodeIt.first->nodeId); fprintf(fout, "S\t%s\t*\n",
nodeId.c_str());
                        }
                }
        }
        ...
        for (auto& path : paths)
        {
                ...
                {
                        std::string nodeId = "node_" +
std::to_string(abs(signedNodeIds.at(nodeLeft))); std::string nodeSign =
signedNodeIds.at(nodeLeft) > 0 ? "+" : "-"; fprintf(fout,
"L\t%s\t%s\t%s\t%s\t0M\n", nodeId.c_str(), nodeSign.c_str(), ctgName.c_str(),
ctgSign.c_str());
                }
                ...
                {
                        std::string nodeId = "node_" +
std::to_string(abs(signedNodeIds.at(nodeRight))); std::string nodeSign =
signedNodeIds.at(nodeRight) > 0 ? "+" : "-"; fprintf(fout,
"L\t%s\t%s\t%s\t%s\t0M\n", ctgName.c_str(), ctgSign.c_str(), nodeId.c_str(),
nodeSign.c_str());
                }
        }
}*/
...
void OutputGenerator::outputDot(const std::vector<UnbranchingPath> &paths,
  ...
  const std::string COLORS[] = {
      "red",          "darkgreen",      "blue",        "goldenrod",
      "cadetblue1",   "darkorchid",     "aquamarine1", "darkgoldenrod1",
  ...
  for (auto &contig : paths) {
    ...
    if (contig.repetitive) {
      ...
      std::string direction =
          contig.path.front()->selfComplement ? ", dir = both" : "";
      fout << "\"" << nodeToId(contig.path.front()->nodeLeft) << "\" -> \""
           << nodeToId(contig.path.back()->nodeRight) << "\" [label = \"id "
           << contig.id.signedId() << "\\l" << lengthStr.str()
