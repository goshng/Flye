...
// Given the reads connecting unique edges (or pairs of edges in the transitions
// graph)
int RepeatResolver::resolveConnections(
    const std::vector<Connection> &connections, float minSupport) {
  std::unordered_map<FastaRecord::Id, std::vector<const Connection *>>
      connectIndex;
  ...
  for (auto &conn : connections) {
    ...
    if (leftEdge->edgeId == rightEdge->edgeId ||
        leftEdge->edgeId == rightEdge->edgeId.rc())
      continue;
  ...
  for (auto lemonAsm : lemonToAsm) {
    ...
    if (mateNode == lemon::INVALID)
      continue;

    FastaRecord::Id leftId = lemonAsm.second;
    FastaRecord::Id rightId = lemonToAsm[graph.id(mateNode)];
    int support =
        edgeWeights[getEdge(graph.nodeFromId(lemonAsm.first), mateNode)];

    if (usedEdges.count(leftId))
      continue;
    usedEdges.insert(rightId);

    float confidence =
        (float)support / (leftCoverage[leftId] + rightCoverage[rightId]);

    Logger::get().debug() << "\tConnection " << leftId.signedId() << "\t"
                          << rightId.rc().signedId() << "\t" << support / 4
                          << "\t" << confidence;
    ...
    uniqueConnections.push_back(
        spanningConnections[spanningConnections.size() / 2]);
  }
  ...
  for (auto &conn : uniqueConnections) {
    ...
    EdgeSequence edgeSeq = _graph.addEdgeSequence(
        _readSeqs.getSeq(conn.readSeq.readId), conn.readSeq.start,
        conn.readSeq.length(), ss.str());

    this->separatePath(conn.path, edgeSeq, edgeId);
    this->separatePath(_graph.complementPath(conn.path), edgeSeq.complement(),
                       edgeId.rc());
  }

  Logger::get().debug() << "[SIMPL] Resolved repeats: "
                        << uniqueConnections.size();
...
bool RepeatResolver::checkForTandemCopies(
    const GraphEdge *checkEdge, const std::vector<GraphAlignment> &alignments) {
...
bool RepeatResolver::checkByReadExtension(
    const GraphEdge *checkEdge, const std::vector<GraphAlignment> &alignments) {
  ...
  for (auto &aln : alignments) {
    ...
    for (size_t i = 0; i < aln.size(); ++i) {
      ...
      if (passedStart && !aln[i].edge->repetitive) {
        if (aln[i].edge->edgeId != checkEdge->edgeId &&
          ...
          int rightFlank = aln.back().overlap.curEnd - aln[i].overlap.curBegin;
    ...
    if (!foundUnique) {
      if (aln.size() > startIndex + 1) {
        hangingPaths.push_back(
            GraphAlignment(aln.begin() + startIndex + 1, aln.end()));
      }
    }
  }
  ...
  if (maxSupport > 1)
    minSupport = std::max(minSupport, 1);
  ...
  if (uniqueMult > 1) {
    Logger::get().debug() << "Starting " << checkEdge->edgeId.signedId()
                          << " aln:" << alignments.size();
    for (auto &outEdgeCount : outFlanks) {
      ...
      Logger::get().debug() << "\t" << star << " " << loop << " " << tip << " "
                            << outEdgeCount.first->edgeId.signedId()
                            << "\tnum:" << outEdgeCount.second.size()
  ...
  // if single unique connection candidate is found, perform an additional
  // consistency check.
  const size_t MIN_SPAN_TO_CHECK = 1;
  if (uniqueMult == 1 && maxConn &&
      outSpans[maxConn].size() >= MIN_SPAN_TO_CHECK) {
    return this->checkPathConsistency(checkEdge, maxConn, visitedEdges,
                                      outSpans, hangingPaths);
  }

  return false;
}

bool RepeatResolver::checkPathConsistency(
    const GraphEdge *checkEdge, GraphEdge *maxConn,
    std::unordered_map<GraphEdge *, std::vector<GraphEdge *>> visitedEdges,
    std::unordered_map<GraphEdge *, std::vector<int>> outSpans,
    std::vector<GraphAlignment> hangingPaths) {
  ...
  // construct a set of safe edges, definted by the read paths between the two
  // uniqe edge candidates
  ...
  {
          Logger::get().debug() << "\t\t" << edgeIt.first->edgeId.signedId() <<
  " " << edgeIt.second;
  }*/

  // for each hanging path, count how many edges differ from the set of safe
  // edges
  ...
  for (auto &aln : hangingPaths) {
    ...
    for (auto &ovlp : aln) {
      if (safeEdgesFrequencies[ovlp.edge] < minSafeFreq) {
        if (ovlp.edge->meanCoverage >= MIN_CUTOFF && !ovlp.edge->altHaplotype) {
    ...
    int unsafeDist = 0;
    for (auto &edge : unsafeEdges)
      unsafeDist += edge->length();
    if (unsafeDist > MIN_UNSAFE_DEVIATION)
      ++inconsistentHangs;
    /*if (!unsafeEdges.empty())
    {
            Logger::get().debug() << "\tInconsistent " <<
    aln.back().overlap.curEnd - aln.front().overlap.curBegin
            ...
            for (auto& ovlp : aln)
            {
                    ...
                    {
                            Logger::get().debug() << "\t\t" <<
    ovlp.edge->edgeId.signedId()
                                    << " " << ovlp.edge->length() << " " <<
    ovlp.edge->meanCoverage;
                    }
            }
    }*/
  }

  // int threshold = std::max(checkEdge->meanCoverage / INCONSISTENT_HANG_RATE,
  // 1);
  int threshold = std::max((outSpans[maxConn].size() + hangingPaths.size()) /
                               INCONSISTENT_HANG_RATE,
                           1UL);
  if (inconsistentHangs > threshold) {
    Logger::get().debug() << "SuspiciousOverhangs: "
                          << checkEdge->edgeId.signedId();
    Logger::get().debug() << "\tSpanning: " << outSpans[maxConn].size()
                          << " median: " << median(outSpans[maxConn])
                          << " max: "
                          << *std::max_element(outSpans[maxConn].begin(),
                                               outSpans[maxConn].end());
...
int RepeatResolver::maskUnsupportedEdges() {
  ...
  for (auto &path : unbranchingPaths) {
    ...
    if (path.meanCoverage < coverageThreshold) {
      Logger::get().debug()
          << "Low-coverage: " << path.edgesStr() << " " << path.meanCoverage;
...
void RepeatResolver::findRepeats() {
  ...
  for (auto &path : unbranchingPaths) {
    ...
    if (!Parameters::get().unevenCoverage &&
      ...
      Logger::get().debug() << "High-cov: " << path.edgesStr() << "\t"
                            << path.length << "\t" << path.meanCoverage;
    }
    ...
    for (auto &edge : path.path) {
      if (!edge->repetitive &&
          this->checkForTandemCopies(edge, alnIndex[edge])) {
        markRepetitive(&path);
        markRepetitive(complPath(&path));
  ...
  for (size_t i = 0; i < numIters; ++i) {
    ...
    for (auto &path : sortedPaths) {
      ...
      // be more aggressive in metagenome mode: assume that edges longer than
      // 50k are unique if (Parameters::get().unevenCoverage && 	path->length >
      //(int)Config::get("unique_edge_length")) continue;

      bool rightRepeat = this->checkByReadExtension(
          path->path.back(), alnIndex[path->path.back()]);
      bool leftRepeat = this->checkByReadExtension(
          complPath(path)->path.back(), alnIndex[complPath(path)->path.back()]);
      if (rightRepeat || leftRepeat) {
        markRepetitive(path);
        markRepetitive(complPath(path));

        Logger::get().debug() << "Mult: " << path->edgesStr() << "\t"
                              << path->length << "\t" << path->meanCoverage
                              << "\t"
                                 " ("
...
void RepeatResolver::finalizeGraph() {
  ...
  for (auto &path : unbranchingPaths) {
    ...
    if (!path.path.front()->selfComplement && path.path.front()->repetitive &&
      ...
      Logger::get().debug() << "Fixed: " << path.edgesStr() << "\t"
                            << path.length << "\t" << path.meanCoverage;
    }
  }
  ...
  for (auto &path : unbranchingPaths) {
    ...
    for (auto &edge : path.path) {
      edge->meanCoverage =
          std::max(0, (int)edge->meanCoverage - _substractedCoverage[edge]);
    }
  }
}
...
int RepeatResolver::resolveRepeats() {
  ...
  auto connections = this->getConnections();
  int resolvedConnections = this->resolveConnections(connections, MIN_SUPPORT);
  this->clearResolvedRepeats();
...
std::vector<RepeatResolver::Connection> RepeatResolver::getConnections() {
  ...
  auto safeEdge = [](GraphEdge *edge) { return !edge->isRepetitive(); };

  int totalSafe = 0;
  for (GraphEdge *edge : _graph.iterEdges()) {
    if (edge->edgeId.strand() && safeEdge(edge))
      ++totalSafe;
  }
    ...
    for (auto &aln : readPath) {
      if (currentAln.empty()) {
        if (!safeEdge(aln.edge))
          continue;
        readStart =
            aln.overlap.curEnd + aln.overlap.extLen - aln.overlap.extEnd;
      ...
      if (safeEdge(aln.edge) && currentAln.front().edge != aln.edge) {
        ...
        if (!currentAln.front().edge->nodeRight->isBifurcation() ||
            !currentAln.back().edge->nodeLeft->isBifurcation())
...
void RepeatResolver::clearResolvedRepeats() {
  ...
  for (auto &node : _graph.iterNodes()) {
    ...
    if (resolvedRepeat) {
      ...
      if (removeLast)
        toRemove.insert(complPath.front()->nodeLeft);
      ...
      // last-first
      if (removeLast)
        toRemove.insert(traversed.back()->nodeRight);
...
int RepeatResolver::resolveSimpleRepeats() {
  ...
  for (auto &pathToResolve : unbranchingPaths) {
    if (!pathToResolve.id.strand())
      continue;
    if (pathToResolve.path.front()->selfComplement)
      continue;
    ...
    std::unordered_set<GraphEdge *> inputs(
        pathToResolve.nodeLeft()->inEdges.begin(),
        pathToResolve.nodeLeft()->inEdges.end());
    std::unordered_set<GraphEdge *> outputs(
        pathToResolve.nodeRight()->outEdges.begin(),
        pathToResolve.nodeRight()->outEdges.end());
    if (inputs.size() != outputs.size() || inputs.size() <= 1 ||
        inputs.size() > MAX_DEGREE)
      continue;

    std::unordered_map<GraphEdge *, std::unordered_map<GraphEdge *, int>>
        readSupport;
    std::unordered_map<GraphEdge *,
                       std::unordered_map<GraphEdge *, ReadSequence>>
        bridgingReads;
        ...
        for (size_t i = 0; i < aln.size(); ++i) {
          ...
          for (size_t j = i + 1; j < aln.size(); ++j) {
            if (outputs.count(aln[j].edge)) {
              ...
              bridgingReads[inEdge][aln[j].edge] = {aln[i].overlap.curId,
                                                    aln[i].overlap.curEnd,
    ...
    for (GraphEdge *inEdge : inputs) {
      for (auto outEdge : readSupport[inEdge]) {
        if (outEdge.second >= MIN_JCT_SUPPORT) {
          unionSet(inputElements[inEdge], outputElements[outEdge.first]);
        }
      }
    }
    ...
    /*if (clusters.size() > 1)
    {
            ...
            for (auto& cl : clusters)
            {
                    ...
                    {
                            Logger::get().debug() << "\t\t" <<
    edgeDir.edge->edgeId.signedId() << " "
                                    << edgeDir.edge->length() << " " <<
    edgeDir.edge->meanCoverage
                                    << " " << edgeDir.isEntrance;
                    }
            }
    }*/
    for (auto &cl : clusters) {
      if (cl.second.size() == 2) {
        ...
        Logger::get().debug() << "\tConnection " << inputConn->edgeId.signedId()
...
void RepeatResolver::separatePath(const GraphPath &graphPath,
  ...
  int32_t pathCoverage =
      (graphPath.front()->meanCoverage + graphPath.back()->meanCoverage) / 2;
  ...
  if (graphPath.size() > 2) {
    ...
    GraphEdge *newEdge = _graph.addEdge(GraphEdge(leftNode, rightNode, newId));
