...
std::vector<GraphAlignment> ReadAligner::chainReadAlignments(
  ...
  for (auto &edgeAlignment : ovlps) {
    ...
    // can't continue, create a new chain
      ...
      if (canBeExtended) {
        activeChains.push_back({{&edgeAlignment}, edgeAlignment.overlap.score});
      } else {
        frozenChains.push_back({{&edgeAlignment}, edgeAlignment.overlap.score});
      }
    }
    ...
    if (numOutdated > (int)activeChains.size() / 2) {
      ...
      while (itCur != activeChains.end()) {
        bool outdated =
            edgeAlignment.overlap.curBegin - itCur->aln.back()->overlap.curEnd >
            MAX_JUMP;
        if (outdated) {
  ...
  std::sort(
      ...
      [](const Chain &c1, const Chain &c2) { return c1.score > c2.score; });

  // greedily choose non-intersecting set of alignments
  std::vector<GraphAlignment> acceptedAlignments;
  for (auto &chain : activeChains) {
    int32_t alnLen =
        chain.aln.back()->overlap.curEnd - chain.aln.front()->overlap.curBegin;
    if (alnLen < MIN_ALN)
      continue;
    ...
    for (auto &existAln : acceptedAlignments) {
      ...
      int32_t overlapRate =
          std::min(curEnd, existEnd) - std::max(curStart, existStart);
      if (overlapRate > MAX_SEP)
        overlaps = true;
    }
    if (!overlaps) {
      acceptedAlignments.emplace_back();
      for (auto &aln : chain.aln)
        acceptedAlignments.back().push_back(*aln);
    }
  }
...
void ReadAligner::alignReads() {
  ...
  // create database
  std::unordered_map<FastaRecord::Id, std::pair<GraphEdge *, EdgeSequence>>
      idToSegment;
  ...
  std::function<void(const FastaRecord::Id &)> alignRead =
      [this, &indexMutex, &numAligned, &readsOverlaps, &idToSegment,
       &alignedLength, &alignedInFull,
       &divergenceStats](const FastaRecord::Id &seqId) {
        ...
        indexMutex.lock();
        ++numAligned;
        if (goodChains.size() == 1)
          ++alignedInFull;
        for (auto &chain : goodChains) {
          chain.shrink_to_fit();
          _readAlignments.push_back(chain);
          alignedLength +=
              chain.back().overlap.curEnd - chain.front().overlap.curBegin;
        }
        for (auto &chain : complChains) {
          chain.shrink_to_fit();
          _readAlignments.push_back(chain);
  ...
  processInParallel(allQueries, alignRead, Parameters::get().numThreads, true);
  ...
  Logger::get().info() << "Aligned read sequence: " << alignedLength << " / "
                       << totalLength << " ("
                       << (float)alignedLength / totalLength << ")";
...
void ReadAligner::updateAlignments() {
  auto isValidAlignment = [this](const GraphAlignment &aln) {
    for (size_t i = 0; i < aln.size() - 1; ++i) {
      if (!_graph.getEdge(aln[i].edge->edgeId) ||
          !_graph.getEdge(aln[i + 1].edge->edgeId))
        return false;
  ...
  _readAlignments.erase(_readAlignments.begin() + insertIdx,
                        _readAlignments.end());
  ...
  for (auto &aln : this->getAlignments()) {
    if (aln.size() > 1) {
      ...
      for (GraphEdge *edge : uniqueEdges)
        alnIndex[edge].push_back(aln);
    }
  }
  return alnIndex;
}

float ReadAligner::getChainBaseDivergence(const GraphAlignment &chain,
                                          bool realign) {
  static const float MAX_DIVERGENCE = Config::get("read_align_ovlp_divergence");
