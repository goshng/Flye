...
void GraphProcessor::fixChimericJunctions() {
  ...
  for (auto &node : _graph.iterNodes()) {
    if (!node->isBifurcation() && (node->inEdges.front()->edgeId ==
  ...
  Logger::get().debug() << "Removed " << simpleCases.size() << " simple and "
                        << complexCases.size() << " double chimeric junctions";
}
...
/*int GraphProcessor::collapseBulges()
{
        ...
        for (auto& edge : _graph.iterEdges())
        {
                ...
                if (parallelEdges[0]->edgeId == parallelEdges[1]->edgeId.rc())
continue; if (parallelEdges[0]->length() > MAX_BUBBLE ||
        ...
        for (auto& nodes : toFix)
        {
                ...
                if (abs(edgeOne->edgeId.signedId()) >
abs(edgeTwo->edgeId.signedId()))
                {
...
int GraphProcessor::condenceEdges() {
  ...
  auto collapseEdges = [](const GraphPath &edges) {
    ...
    for (size_t i = 1; i < edges.size(); ++i) {
      ...
      for (auto prevSeg = growingSeqs.begin(); prevSeg != growingSeqs.end();) {
      ...
      if (growingSeqs.empty()) {
        ...
        std::copy(edges[i]->seqSegments.begin(), edges[i]->seqSegments.end(),
                  std::back_inserter(growingSeqs));
        prevStart = i;
      }
    }

    newEdges.emplace_back(edges[prevStart]->nodeLeft, edges.back()->nodeRight);
  ...
  for (auto &unbranchingPath : toCollapse) {
    ...
    std::unordered_set<GraphEdge *> toRemove;
    for (auto &edge : unbranchingPath.path)
      toRemove.insert(edge);
    for (auto &edge : complPath)
      toRemove.insert(edge);
    for (auto &edge : toRemove)
  ...
  Logger::get().debug() << "Collapsed " << edgesRemoved - edgesAdded
                        << " edges";
...
std::vector<UnbranchingPath> GraphProcessor::getEdgesPaths() const {
  ...
  for (auto &edge : _graph.iterEdges()) {
    GraphPath path = {edge};
    paths.emplace_back(path, edge->edgeId, false, edge->length(),
                       edge->meanCoverage, "edge_");
...
std::vector<UnbranchingPath> GraphProcessor::getUnbranchingPaths() const {
  ...
  for (auto edge : _graph.iterEdges()) {
    ...
    if (!edge->selfComplement) {
      ...
      while (!curNode->isBifurcation() && !curNode->inEdges.empty() &&
      ...
      while (!curNode->isBifurcation() && !curNode->outEdges.empty() &&
    ...
    bool circular =
        (traversed.front()->nodeLeft == traversed.back()->nodeRight) &&
        traversed.front()->nodeLeft->outEdges.size() == 1 &&
        traversed.front()->nodeLeft->inEdges.size() == 1;

    bool repetitive =
        traversed.front()->isRepetitive() || traversed.back()->isRepetitive();
    ...
    unbranchingPaths.emplace_back(traversed, edgeId, circular, contigLength,
                                  meanCoverage);
