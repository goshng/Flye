...
void sortByKey(std::vector<T> &container, KeyFn keyFn) {
  std::sort(
      ...
      [keyFn](const T &k1, const T &k2) { return keyFn(k1) < keyFn(k2); });
}
} // namespace
...
bool GraphEdge::isRightTerminal() const {
  ...
  return this->nodeRight->outEdges.size() == 0 &&
         this->nodeRight->inEdges.size() == 1;
}

std::unordered_set<GraphEdge *> GraphEdge::adjacentEdges() {
  std::unordered_set<GraphEdge *> edges;
  for (auto &e : nodeLeft->inEdges)
    edges.insert(e);
  for (auto &e : nodeLeft->outEdges)
    edges.insert(e);
  for (auto &e : nodeRight->inEdges)
    edges.insert(e);
  for (auto &e : nodeRight->outEdges)
    edges.insert(e);
  edges.erase(this);
  return edges;
}
...
void RepeatGraph::build(bool keepHaplotypes) {
  ...
  OverlapDetector asmOverlapper(
      _asmSeqs, asmIndex, (int)Config::get("maximum_jump"),
...
void RepeatGraph::getGluepoints(OverlapContainer &asmOverlaps) {
  ...
  for (auto &seq : _asmSeqs.iterSeqs()) {
    for (auto &ovlp : asmOverlaps.lazySeqOverlaps(seq.id)) {
      ...
      endpoints[ovlp.curId].push_back(new SetPoint2d(
          Point2d(ovlp.curId, ovlp.curEnd, ovlp.extId, ovlp.extEnd)));
    }
  }
  ...
  for (auto &seqPoints : endpoints) {
    sortByKey(seqPoints.second,
              [](SetPoint2d *const &p) { return p->data.curPos; });
  ...
  for (auto &clustEndpoints : clusters) {
    ...
    for (auto &interval : asmOverlaps.getCoveringOverlaps(
             clustSeq, clusterXpos - 1, clusterXpos + 1)) {
      ...
      if (ovlp.curEnd - clusterXpos > _maxSeparation &&
        ...
        extCoords.push_back(new SetPoint2d(
            Point2d(clustSeq, clusterXpos, ovlp.extId, projectedPos)));
      }
    }
    ...
    for (auto &clustPt : clusterPoints) {
      ...
      size_t i = std::lower_bound(seqGluepoints.begin(), seqGluepoints.end(),
                                  clustPt.pos, cmp) -
                 seqGluepoints.begin();
      ...
      size_t ci = std::upper_bound(complGluepoints.begin(),
                                   complGluepoints.end(), complPt.pos, cmp2) -
                  complGluepoints.begin();
      ...
      seqGluepoints.insert(seqGluepoints.begin() + i, new SetPoint1d(clustPt));
    ...
    for (size_t i = 0; i < toMerge.size() - 1; ++i) {
      ...
      unionSet(complements[toMerge[i]], complements[toMerge[i + 1]]);
    }
  }
  ...
  auto addConsensusPoint = [&setToId, this,
                            &pointId](const std::vector<SetPoint1d *> &group) {
    SetPoint1d *reprPoint = group.front();
    if (!setToId.count(findSet(reprPoint))) {
      setToId[findSet(reprPoint)] = pointId++;
    }
    int32_t clusterSize = group.back()->data.pos - group.front()->data.pos;

    // big cluster corresponding to a tandem repeat -
    // split it into multiple short edges
    if (clusterSize > _maxSeparation) {
      _gluePoints[reprPoint->data.seqId].emplace_back(
          setToId[findSet(reprPoint)], reprPoint->data.seqId,
      ...
      for (int32_t i = 1; i < repeats; ++i) {
        ...
        _gluePoints[reprPoint->data.seqId].emplace_back(
            setToId[findSet(reprPoint)], reprPoint->data.seqId, pos);
      }
    ...
    //"normal" endpoint - just take a consensus
      ...
      _gluePoints[reprPoint->data.seqId].emplace_back(
          setToId[findSet(reprPoint)], reprPoint->data.seqId, clusterXpos);
    }
  };
  ...
  for (auto &seq : _asmSeqs.iterSeqs()) {
    ...
    if (seqPoints.empty() || seqPoints.front().position > MAX_TIP) {
      seqPoints.emplace(seqPoints.begin(), pointId++, seq.id, 0);
      complPoints.emplace_back(pointId++, seq.id.rc(),
                               _asmSeqs.seqLen(seq.id) - 1);
    }
    if (seqPoints.size() == 1 ||
        _asmSeqs.seqLen(seq.id) - seqPoints.back().position > MAX_TIP) {
      seqPoints.emplace_back(pointId++, seq.id, _asmSeqs.seqLen(seq.id) - 1);
      complPoints.emplace(complPoints.begin(), pointId++, seq.id.rc(), 0);
    }
  }

  int numGluepoints = 0;
  for (auto &seqRec : _gluePoints)
    numGluepoints += seqRec.second.size();
...
void RepeatGraph::checkGluepointProjections(
    const OverlapContainer &asmOverlaps) {
  size_t MAX_ITER = 100;
  for (size_t i = 0; i < MAX_ITER; ++i) {
    std::unordered_map<FastaRecord::Id, std::vector<GluePoint>> addedGluepoints;
    ...
    auto combinePts = [&mergedGluepoints](size_t idOne, size_t idTwo) {
      ...
      unionSet(mergedGluepoints[idOne], mergedGluepoints[idTwo]);
    };
    ...
    for (auto &seq : _asmSeqs.iterSeqs()) {
      ...
      for (size_t i = 0; i < gp.size(); ++i) {
        GluePoint pt = gp[i];
        GluePoint ptCompl = _gluePoints[pt.seqId.rc()][gp.size() - i - 1];

        for (auto &interval : asmOverlaps.getCoveringOverlaps(
                 seq.id, pt.position - 1, pt.position + 1)) {
          auto &ovlp = *interval.value;
          ...
          for (; rBegin != rEnd; ++rBegin) {
            size_t compPos =
                seqPoints.size() - (rBegin - seqPoints.begin()) - 1;
            GluePoint projCompl = _gluePoints[ovlp.extId.rc()][compPos];

            if (abs(rBegin->position - projectedPos) <= _maxSeparation) {
    ...
    for (auto &ptVec : addedGluepoints) {
      ...
      sortByKey(permutation,
                [&ptVec](size_t const &i) { return ptVec.second[i].position; });
      ...
      sortByKey(_gluePoints[ptVec.first],
                [](const GluePoint &gp) { return gp.position; });
      sortByKey(_gluePoints[ptVec.first.rc()],
                [](const GluePoint &gp) { return gp.position; });
    }
    Logger::get().debug() << "Added " << totalAdded << " gluepoint projections";
    ...
    for (auto &seq : _asmSeqs.iterSeqs()) {
      ...
      for (auto &point : _gluePoints[seq.id]) {
        if (mergedGluepoints.count(point.pointId)) {
          point.pointId = findSet(mergedGluepoints[point.pointId])->data;
        }
      }
    }
...
void RepeatGraph::collapseTandems() {
  ...
  for (auto &seqPoints : _gluePoints) {
    ...
    while (rightId < seqPoints.second.size()) {
      ...
      if (rightId < seqPoints.second.size()) {
        tandemRights[tandemId].insert(seqPoints.second[rightId].pointId);
      } else {
        tandemRights[tandemId].insert(-1);
      }
      if (leftId > 0) {
        tandemLefts[tandemId].insert(seqPoints.second[leftId - 1].pointId);
      } else {
  ...
  Logger::get().debug() << "Artificial loops removed: " << collapsedLeft
                        << " left, " << collapsedRight << " right, "
                        << collapsedBoth << " both";
}
...
void RepeatGraph::initializeEdges(const OverlapContainer &asmOverlaps) {
  ...
  std::unordered_map<NodePair, std::vector<EdgeSequence>, pairhash>
      parallelSegments;
  ...
  for (auto &seq : _asmSeqs.iterSeqs()) {
    ...
    for (size_t i = 0; i < seqGluepoints.size() - 1; ++i) {
      ...
      parallelSegments[revPair].push_back(
          parallelSegments[fwdPair].back().complement());

      complEdges[fwdPair] = revPair;
      complEdges[revPair] = fwdPair;
      // Logger::get().debug() << checksum << " " << gpRight.position -
      // gpLeft.position;
  ...
  auto segIntersect = [](const EdgeSequence &s, int32_t intBegin,
                         int32_t intEnd) {
    return std::max(
        std::min(intEnd, s.origSeqEnd) - std::max(intBegin, s.origSeqStart), 0);
  };

  // sort nodes wrt to their ids to make it deterministic
  std::vector<NodePair> sortedKeys;
  for (auto &it : parallelSegments)
  ...
  for (auto &nodePair : sortedKeys) {
    ...
    std::unordered_map<FastaRecord::Id, std::vector<SetSegment *>> segmentIndex;
    ...
    for (auto &seqSegments : segmentIndex) {
      sortByKey(seqSegments.second,
                [](SetSegment *const &s) { return s->data->origSeqStart; });
    }

    // cluster segments based on their overlaps
    for (auto &setOne : segmentSets) {
      for (auto &interval : asmOverlaps.getCoveringOverlaps(
               setOne->data->origSeqId, setOne->data->origSeqStart,
        ...
        auto startRange =
            std::lower_bound(ss.begin(), ss.end(), ovlp.extBegin, cmpEnd);
        auto endRange =
            std::lower_bound(ss.begin(), ss.end(), ovlp.extEnd, cmpBegin);
        if (endRange != ss.end())
          ++endRange;
        for (; startRange != endRange; ++startRange) {
          auto *setTwo = *startRange;
          if (findSet(setOne) == findSet(setTwo))
            continue;
    ...
    {
            Logger::get().debug() << "Node with " << segmentSets.size() << "
    segments"; Logger::get().debug() << "clusters: " << edgeClusters.size(); for
    (auto& edgeClust : edgeClusters)
            {
                    int sumLen = 0;
    ...
    segOne->origSeqStart, segOne->origSeqEnd))
                                            {
                                                    auto& ovlp =
    *interval.value; int32_t intersectOne = segIntersect(*segOne, ovlp.curBegin,
    ovlp.curEnd); if (intersectOne < segOne->seqLen / 2) continue;

                                                    auto& ss =
    segmentIndex[ovlp.extId]; auto cmpBegin = [] (const SetSegment* s, int32_t
    pos) {return s->data->origSeqStart < pos;}; auto cmpEnd = [] (const
    SetSegment* s, int32_t pos) {return s->data->origSeqEnd < pos;}; auto
    startRange = std::lower_bound(ss.begin(), ss.end(), ovlp.extBegin, cmpEnd);
                                                    auto endRange =
    std::lower_bound(ss.begin(), ss.end(), ovlp.extEnd, cmpBegin); if (endRange
    != ss.end()) ++endRange; for (;startRange != endRange; ++startRange)
                                                    {
                                                            auto* setTwo =
    *startRange; if (segOne->origSeqStart == setTwo->data->origSeqStart &&
                                                                    segOne->origSeqEnd
    == setTwo->data->origSeqEnd) continue;

                                                            //projecting the
    interval endpoints
                                                            //(overlap might be
    covering the actual segment) int32_t projStart =
    ovlp.project(segOne->origSeqStart); int32_t projEnd =
    ovlp.project(segOne->origSeqEnd); int32_t projIntersect =
                                                                    segIntersect(*setTwo->data,
    projStart, projEnd);
    ...
    std::unordered_map<SetSegment *, std::pair<FastaRecord::Id, int32_t>>
        sortOrder;
    for (auto &cl : edgeClusters) {
      EdgeSequence *minEdge = *std::min_element(
      ...
      sortOrder[cl.first] =
          std::make_pair(minEdge->origSeqId, minEdge->origSeqStart);
    }
    std::vector<SetSegment *> sortedKeysCl;
    for (auto &it : edgeClusters)
      sortedKeysCl.push_back(it.first);
    sortByKey(sortedKeysCl,
              [&sortOrder](SetSegment *const n) { return sortOrder[n]; });
    ...
    for (auto &clustId : sortedKeysCl) {
      ...
      if (edgeClusters.size() > 1 && matchEdges.size() == 1) {
        ...
        bool covered = false;
        for (auto &interval : asmOverlaps.getCoveringOverlaps(
                 seg->origSeqId, seg->origSeqStart, seg->origSeqEnd)) {
          auto &ovlp = *interval.value;
          int32_t intersect = segIntersect(*seg, ovlp.curBegin, ovlp.curEnd);
          if (intersect == seg->seqLen)
            covered = true;
        }
        if (covered) {
          ++singletonsFiltered;
      ...
      bool selfComplement = std::find(usedSegments.begin(), usedSegments.end(),
                                      anySegment) != usedSegments.end();
      ...
      if (!selfComplement) {
        ...
        GraphEdge *complEdge = this->addEdge(
            GraphEdge(leftNode, rightNode, FastaRecord::Id(_nextEdgeId + 1)));
        for (auto &seg : matchEdges) {
...
void RepeatGraph::logEdges() {
  ...
  std::unordered_map<FastaRecord::Id, std::vector<SegEdgePair>> sequenceEdges;
  ...
  for (auto &seqEdgesPair : sequenceEdges) {
    sortByKey(seqEdgesPair.second,
              [](const SegEdgePair &s) { return s.first->origSeqStart; });
  }
  ...
  for (auto &seq : _asmSeqs.iterSeqs()) {
    ...
    while (begin < seqEdgesVec.size()) {
      size_t end = begin + 1;
      while (end < seqEdgesVec.size() && seqEdgesVec[begin].second->edgeId ==
                                             seqEdgesVec[end].second->edgeId)
        ++end;
      ...
      std::string mult =
          end - begin == 1 ? "" : "(" + std::to_string(end - begin) + ")";
      Logger::get().debug()
          << unique << "\t" << edge->edgeId.signedId() << "\t"
          << _asmSeqs.seqName(beginSeg->origSeqId) << "\t"
          << beginSeg->origSeqStart << "\t" << endSeg->origSeqEnd << "\t"
...
void RepeatGraph::storeGraph(const std::string &filename) {
  ...
  for (auto &edge : this->iterEdges()) {
    fout << "Edge\t" << edge->edgeId << "\t" << nodeIds[edge->nodeLeft] << "\t"
         << nodeIds[edge->nodeRight] << "\t" << edge->repetitive << "\t"
         << edge->selfComplement << "\t" << edge->resolved << "\t"
...
void RepeatGraph::validateGraph() {
  ...
  for (auto &edge : this->iterEdges()) {
    if (!std::count(edge->nodeLeft->outEdges.begin(),
      ...
      Logger::get().warning()
          << "Inconsistent node-edge reference " << edge->edgeId.signedId();
    }
...
void RepeatGraph::loadGraph(const std::string &filename) {
  ...
  while (true) {
    ...
    if (buffer == "Edge") {
      ...
      fin >> edge.repetitive >> edge.selfComplement >> edge.resolved >>
          edge.meanCoverage >> edge.altGroupId;
...
void RepeatGraph::updateEdgeSequences() {
  for (auto &edge : this->iterEdges()) {
    ...
    for (const auto &edgeSeq : segmentsToAdd) {
      if (edge->selfComplement && !edgeSeq.origSeqId.strand())
        continue;

      size_t len = edgeSeq.origSeqEnd - edgeSeq.origSeqStart;
      auto subSeq =
          _asmSeqs.getSeq(edgeSeq.origSeqId).substr(edgeSeq.origSeqStart, len);
      std::string description =
          "edge_" + std::to_string(edge->edgeId.signedId()) + "_" +
          std::to_string(num++) + "_" +
