...
void MultiplicityInferer::estimateCoverage() {
  ...
  for (auto &path : _aligner.getAlignments()) {
    for (size_t pathId = 0; pathId < path.size(); ++pathId) {
      ...
      int covTo =
          std::min((int)edgeCov.size(), path[pathId].overlap.extEnd / WINDOW);
  ...
  for (auto edge : _graph.iterEdges()) {
    if (wndCoverage[edge].empty())
      continue;

    GraphEdge *complEdge = _graph.complementEdge(edge);
    int32_t medianCov =
        (median(wndCoverage[edge]) + median(wndCoverage[complEdge])) / 2;
    ...
    Logger::get().debug() << edge->edgeId.signedId()
                          << "\tlen:" << edge->length() << "\tcov:" << medianCov
                          << "\tmult:" << (float)medianCov / _meanCoverage;
...
int MultiplicityInferer::resolveForks() {
  ...
  for (GraphNode *node : originalNodes) {
    ...
    if (node->inEdges.size() != 1 || node->outEdges.size() != 2)
      continue;
    ...
    if (inEdge->selfComplement || inEdge->isLooped() ||
        ...
        outMinor->selfComplement || outMinor->isLooped())
      continue;
    ...
    if (std::min(outMajor->meanCoverage, inEdge->meanCoverage) <
        outMinor->meanCoverage * MAJOR_TO_MINOR)
      continue;

    // looks like all is good
    Logger::get().debug() << "Disconnected fork: "
                          << outMinor->edgeId.signedId();
...
int MultiplicityInferer::disconnectMinorPaths() {
  ...
  auto nodeDegree = [](GraphNode *node) {
    ...
    for (auto& edge : node->outEdges)
    {
            if (!edge->isLooped()) maxOut = std::max(maxOut,
    edge->meanCoverage);
    }
  ...
  for (auto &path : unbranchingPaths) {
    if (!path.id.strand() || path.isLooped() ||
        path.path.front()->selfComplement || path.length > MAX_LEN)
      continue;
    if (path.nodeLeft()->inEdges.empty() || path.nodeRight()->outEdges.empty())
      continue; // already detached or tip

    bool weakLeft =
        path.nodeLeft()->inEdges.empty() ||
        nodeDegree(path.nodeLeft()) > path.meanCoverage * DETACH_RATE;
    bool weakRight =
        path.nodeRight()->outEdges.empty() ||
        nodeDegree(path.nodeRight()) > path.meanCoverage * DETACH_RATE;
    if (weakLeft && weakRight)
      toRemove.insert(path.id);
  }
  ...
  _aligner.updateAlignments();
  Logger::get().debug() << "[SIMPL] Disconnected " << numDisconnected
                        << " minor paths";
...
int MultiplicityInferer::splitNodes() {
  ...
  for (auto &readPath : _aligner.getAlignments()) {
    ...
    for (size_t i = 0; i < readPath.size() - 1; ++i) {
      ...
      if (readPath[i].edge->edgeId == readPath[i + 1].edge->edgeId.rc())
        continue;
  ...
  for (auto &nodeToSplit : originalNodes) {
    if (nodeToSplit->inEdges.size() < 2 || nodeToSplit->outEdges.size() < 2)
      continue;
    if (usedNodes.count(nodeToSplit))
      continue;
    ...
    if (clusters.size() > 1) // need to split the node!
    {
      ...
      for (auto &cl : clusters) {
        ...
        for (auto edgeDir : cl.second) {
          ...
          if (!selfComplNode) {
            switchNode(complEdge, newComplNode, !edgeDir.isInput);
          }
        }
      }
    }
...
int MultiplicityInferer::removeUnsupportedConnections() {
  ...
  int numDisconnected = 0;
  for (auto &edge : _graph.iterEdges()) {
    if (!edge->edgeId.strand() || edge->isLooped())
      continue;
    GraphEdge *complEdge = _graph.complementEdge(edge);
    ...
    if (!edge->nodeRight->isEnd() && edge->nodeRight->isBifurcation() &&
        rightConnections[edge] / 2 < coverageThreshold) {
      ++numDisconnected;
      Logger::get().debug() << "Chimeric right: " << edge->edgeId.signedId()
                            << " " << rightConnections[edge] / 2;

      _graph.disconnectRight(edge);
      _graph.disconnectLeft(complEdge);

      if (edge->selfComplement)
        continue; // already discinnected
    }
    if (!edge->nodeLeft->isEnd() && edge->nodeLeft->isBifurcation() &&
        leftConnections[edge] / 2 < coverageThreshold) {
      ++numDisconnected;
      Logger::get().debug() << "Chimeric left: " << edge->edgeId.signedId()
                            << " " << leftConnections[edge] / 2;
...
void MultiplicityInferer::trimTipsIteration(int &outShort, int &outLong) {
  ...
  for (auto &tipPath : unbranchingPaths) {
    if (!tipPath.path.back()->isRightTerminal())
      continue; // right-tip
    if (tipPath.nodeLeft()->outEdges.size() == 1)
      continue; // already detached from the left
    if (tipPath.path.front()->selfComplement)
      continue; // never-ever!
    ...
    for (GraphEdge *edge : tipNode->outEdges) {
      ...
      if (path.path.front() == edge) {
        if (path.length > LEN_RATE * tipPath.length ||
            path.nodeRight()->outEdges.size() > 0)
    ...
    int trueCov =
        std::max(entrances.front()->meanCoverage, exits.front()->meanCoverage);
  ...
  for (auto &path : unbranchingPaths) {
    if (toRemove.count(path.id)) {
      Logger::get().debug()
          << "Tip " << path.edgesStr() << " len:" << path.length
          << " cov:" << path.meanCoverage;
