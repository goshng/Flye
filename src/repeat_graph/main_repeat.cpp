...
bool parseArgs(int argc, char **argv, std::string &readsFasta,
  ...
  int optionIndex = 0;
  static option longOptions[] = {{"disjointigs", required_argument, 0, 0},
  ...
  if (readsFasta.empty() || outFolder.empty() || inAssembly.empty() ||
      configPath.empty()) {
    printUsage();
    return false;
  }
...
int repeat_main(int argc, char **argv) {
  ...
  outGen.outputFasta(proc.getEdgesPaths(),
                     outFolder + "/graph_before_rr.fasta");
  ...
  if (!keepHaplotypes) {
    ...
    // if we keeping haplotypes, update them one more time to ensure it's
    // consistent with the final graph structure
    markHaplotypes();
  }
