...
int HaplotypeResolver::findHeterozygousBulges() {
  ...
  for (auto &path : unbranchingPaths) {
    ...
    if (twoPaths[0]->id == twoPaths[1]->id.rc())
      continue;
    if (usedPaths.count(twoPaths[0]->id) || usedPaths.count(twoPaths[1]->id))
      continue;
    ...
    if (std::max(twoPaths[0]->length, twoPaths[1]->length) > MAX_BUBBLE_LEN)
      continue;
    ...
    //					  exitPath->meanCoverage * MAX_COV_VAR))
    //continue;
    ...
    if (!twoPaths[0]->path.front()->altHaplotype ||
        !twoPaths[1]->path.front()->altHaplotype)
      ++numMasked;
    ...
    Logger::get().debug() << "\tInternal: " << twoPaths[0]->edgesStrLong()
                          << " " << twoPaths[1]->edgesStrLong();

    if (inEdge->rightLink || outEdge->leftLink)
      continue;
  ...
  Logger::get().debug() << "[SIMPL] Masked " << numMasked << " simple bubbles";
  return numMasked;
}
...
int HaplotypeResolver::findHeterozygousLoops() {
  ...
  for (auto &loop : unbranchingPaths) {
    ...
    if (!loop.isLooped())
      continue;
    if (loop.path.front()->selfComplement)
      continue;
    if (loop.length > MAX_LOOP_LEN)
      continue;

    GraphNode *node = loop.nodeLeft();
    if (node->inEdges.size() != 2 || node->outEdges.size() != 2)
      continue;
    ...
    if (loop.meanCoverage >
        COV_MULT * std::min(entrancePath->meanCoverage, exitPath->meanCoverage))
      continue;

    // loop should not be longer than other branches
    if (loop.length > std::max(entrancePath->length, exitPath->length))
      continue;

    if (!loop.path.front()->altHaplotype)
      ++numMasked;
    ...
    int32_t mainPathCoverage =
        (entrancePath->meanCoverage + exitPath->meanCoverage) / 2;
    int32_t LOOP_COV_RATE =
        4; // 2 times lower than half coverage (in case of haploid indel)
...
HaplotypeResolver::VariantPaths HaplotypeResolver::findVariantSegment(
    GraphEdge *startEdge, const std::vector<GraphAlignment> &alingnments,
  ...
  for (auto &aln : alingnments) {
    for (size_t i = 0; i < aln.size(); ++i) {
      ...
      if (aln[i].edge == startEdge && i + 1 < aln.size()) {
        outPaths.emplace_back(GraphAlignment(aln.begin() + i, aln.end()));
        break;
      }
    }
  }
  ...
  for (auto &trgPath : outPaths) {
    ...
    for (auto &referencePath : pathGroups) {
      bool contained = true;
      for (size_t i = 0;
           i < std::min(trgPath.size(), referencePath.path.size()); ++i) {
  ...
  pathGroups.erase(
      std::remove_if(pathGroups.begin(), pathGroups.end(),
                     [](PathWithScore &p) { return p.score < MIN_SCORE; }),
      pathGroups.end());
  ...
  if (!convergenceEdges.count(refPath.path[bubbleStartId].edge))
    return VariantPaths();
  ...
  for (size_t groupId = 0; groupId < pathGroups.size(); ++groupId) {
    ...
    size_t groupEnd = 0;
    for (size_t i = 0; i < pathGroups[groupId].path.size(); ++i) {
      if (pathGroups[groupId].path[i].edge == refPath.path[bubbleStartId].edge)
        groupStart = i;

      if (pathGroups[groupId].path[i].edge == refPath.path[bubbleEndId].edge)
        groupEnd = i;
    }
    ...
    for (size_t branchId = 0; branchId < bubbleBranches.size(); ++branchId) {
      if (newBranch.path.size() != bubbleBranches[branchId].path.size())
        continue;
    ...
    if (!duplicate)
      bubbleBranches.push_back(newBranch);
  }
  if (bubbleBranches.size() < 2)
    return VariantPaths();
  ...
  if (bridgingReads.empty())
    throw std::runtime_error("No bridging reads!");
  ...
  vp.bridgingSequence =
      _readSeqs.getSeq(readId).substr(readStart, readEnd - readStart);
...
int HaplotypeResolver::findRoundabouts() {
  ...
  for (auto &startPath : unbranchingPaths) {
    ...
    if (loopedEdges.count(startEdge))
      continue;
    if (usedEdges.count(startEdge))
      continue;

    auto varSeg =
        this->findVariantSegment(startEdge, alnIndex[startEdge], loopedEdges);
    if (varSeg.startEdge && varSeg.endEdge &&
        varSeg.startEdge != _graph.complementEdge(varSeg.endEdge)) {
      auto revSeg = this->findVariantSegment(
          _graph.complementEdge(varSeg.endEdge),
          alnIndex[_graph.complementEdge(varSeg.endEdge)], loopedEdges);
  ...
  for (auto &varSegment : foundVariants) {
    ...
    for (auto &branch : varSegment.altPaths) {
      for (size_t i = 1; i < branch.path.size() - 1; ++i) {
        if (branch.path[i].edge->altHaplotype)
          newVariant = false;
      }
    }
    if (newVariant) {
      ++foundNew;
      Logger::get().debug()
          << "Roundabout: " << varSegment.startEdge->edgeDescrLong() << " : "
    ...
    for (auto &branch : varSegment.altPaths) {
      for (size_t i = 1; i < branch.path.size() - 1; ++i) {
        ...
        _graph.complementEdge(branch.path[i].edge)->altGroupId =
            _nextAltGroupId + 1;
      }
    }
    _nextAltGroupId += 2;

    if (varSegment.startEdge->rightLink || varSegment.endEdge->leftLink)
      continue;
...
void HaplotypeResolver::collapseHaplotypes() {
  ...
  for (auto &inEdge : _graph.iterEdges()) {
    if (!inEdge->rightLink)
      continue;
    if (separatedEdges.count(inEdge))
      continue;
  ...
  for (auto& path : unbranchingPaths)
  {
          ...
          for (auto& edge : path.path)
          {
                  ...
                  {
                          //Logger::get().debug() << "Uncircularized: " <<
  path.edgesStr(); _graph.disconnectRight(path.path.back());
...
DnaSequence HaplotypeResolver::pathSequence(GraphPath &path) {
  ...
  for (size_t i = 0; i < path.size(); ++i) {
    strSeq += _graph.edgeSequences()
                  .getSeq(path[i]->seqSegments.front().edgeSeqId)
                  .str();
  }
  if (strSeq.empty())
    strSeq = "A";
...
void HaplotypeResolver::separeteAdjacentEdges(GraphEdge *inEdge,
                                              GraphEdge *outEdge) {
...
void HaplotypeResolver::separateDistantEdges(GraphEdge *inEdge,
                                             GraphEdge *outEdge,
                                             EdgeSequence insertSeq,
  ...
  int32_t pathCoverage = (inEdge->meanCoverage + outEdge->meanCoverage) / 2;
...
auto anyPath = [](GraphEdge *startEdge, int maxDepth,
  ...
  while (!queue.empty()) {
    ...
    for (auto &nextEdge : curPath.path.back()->nodeRight->outEdges) {
      ...
      bool localComplRepeat =
          std::find(curPath.path.rbegin(), curPath.path.rend(),
                    graph.complementEdge(nextEdge)) != curPath.path.rend();
      if (localRepeat || localComplRepeat)
        continue;
      if (nextEdge->isLooped() && nextEdge->length() < maxDepth)
        continue;
      deadEnd = false;
    ...
    if (deadEnds.size() > MAX_CANDIDATES)
      break;
  ...
  for (size_t i = 1; i < deadEnds.size(); ++i) {
    if (deadEnds[i].length > deadEnds[maxId].length)
      maxId = i;
  }
...
auto lowestPri = [](const EdgeWithPriority &e1, const EdgeWithPriority &e2) {
  return e1.priority != e2.priority ? e1.priority > e2.priority
                                    : e1.edge > e2.edge;
};
...
auto getShortestPathsLen = [](GraphEdge *source, GraphEdge *sink, int maxBubble,
                              DijkstraResult &outRes) {
  outRes.dist.clear();
...
Superbubble
isRightSuperbubble(GraphEdge *startEdge, int maxBubbleLen,
                   const RepeatGraph &graph,
                   const std::unordered_set<GraphEdge *> loopedEdges) {
  ...
  for (GraphEdge *endCand : refPath) {
    ...
    for (auto &edgeDist : distancesFromSource.dist) {
      ...
      int tourLen = edgeDist.second + distancesFromSink.dist[complEdge] -
                    complEdge->length();
    ...
    if (goodBubble) {
      ...
      sb.refPath = refPath;
      for (auto &edgeDist : distancesFromSource.dist) {
        if (edgeDist.first != sb.start && edgeDist.first != sb.end)
          sb.internalEdges.insert(edgeDist.first);
      }
...
int HaplotypeResolver::findSuperbubbles() {
  ...
  for (auto &startEdge : _graph.iterEdges()) {
    ...
    // Logger::get().debug() << "\tChecking start: " <<
    ...
    auto fwdBubble =
        isRightSuperbubble(startEdge, MAX_BUBBLE_LEN, _graph, loopedEdges);
    if (!fwdBubble.end || startEdge == fwdBubble.end ||
        startEdge == _graph.complementEdge(fwdBubble.end))
      continue;

    // prohibit complex loops to be called as superbubbles
    if (startEdge->nodeRight == fwdBubble.end->nodeLeft)
      continue;
    ...
    /*auto revBubble = isRightSuperbubble(_graph.complementEdge(fwdBubble.end),
                                                                            MAX_BUBBLE_LEN,
    _graph, loopedEdges); if (!revBubble.end || startEdge !=
    _graph.complementEdge(revBubble.end))
    {
    ...
    if (startEdge->rightLink || fwdBubble.end->leftLink)
      continue;
    ...
    GraphPath bridgePath(fwdBubble.refPath.begin() + 1,
                         std::find(fwdBubble.refPath.begin(),
                                   fwdBubble.refPath.end(), fwdBubble.end));
  ...
  Logger::get().debug() << "[SIMPL] Masked " << foundNew << " superbubbles";
