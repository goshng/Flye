...
void pairwiseAlignment(const std::string &seqOne, const std::string &seqTwo,
                       const SubstitutionMatrix &subsMat, std::string &outOne,
                       std::string &outTwo) {
  ...
  for (size_t i = 0; i < seqOne.length(); ++i) {
    scoreMat.at(i + 1, 0) =
        scoreMat.at(i, 0) + subsMat.getScore(seqOne[i], '-');
    backtrackMat.at(i + 1, 0) = 1;
  }
  for (size_t i = 0; i < seqTwo.length(); ++i) {
    scoreMat.at(0, i + 1) =
        scoreMat.at(0, i) + subsMat.getScore('-', seqTwo[i]);
  ...
  for (size_t i = 1; i < seqOne.length() + 1; ++i) {
    for (size_t j = 1; j < seqTwo.length() + 1; ++j) {
      AlnScoreType left =
          scoreMat.at(i, j - 1) + subsMat.getScore('-', seqTwo[j - 1]);
      AlnScoreType up =
          scoreMat.at(i - 1, j) + subsMat.getScore(seqOne[i - 1], '-');
...
splitBranchHopos(const std::string &candAln, const std::string &branchAln) {
  ...
  std::vector<std::pair<HopoMatrix::State, HopoMatrix::Observation>> result;
  size_t prevPos = 0;
  while (candAln[prevPos] == '-')
    ++prevPos;
  ...
  for (size_t pos = prevPos + 1; pos < candAln.length(); ++pos) {
    ...
    if (candAln[pos] == '-') {
      ...
      if (candAln[pos] != prevNucl) {
        ...
        ++hopoCount;
        // wobble
        size_t branchPrevPos =
            prevPos -
            (size_t)(prevPos > 0 && branchAln[prevPos - 1] == candAln[prevPos]);
        size_t branchPos = pos + (size_t)(branchAln[pos] == candAln[pos - 1]);

        /*if (prevPos > 0 && prevPos < branchAln.length() - 1)
        {
                std::string branchSubseq(branchAln, prevPos - 1, pos - prevPos +
        2); std::string candSubseq(candAln, prevPos - 1, pos - prevPos + 2);
        ...
        auto observ = HopoMatrix::strToObs(state.nucl, branchAln, branchPrevPos,
                                           branchPos);
...
void HomoPolisher::polishBubble(Bubble &bubble) const {
  ...
  for (auto &branch : bubble.branches) {
    ...
    pairwiseAlignment(bubble.candidate, branch, _subsMatrix, alnCand,
                      alnBranch);
...
AlnScoreType
HomoPolisher::likelihood(HopoMatrix::State state,
...
size_t
HomoPolisher::mostLikelyLen(char nucleotide,
                            const HopoMatrix::ObsVector &observations) const {
...
size_t
HomoPolisher::compareTopTwo(char nucleotide, size_t firstChoice,
                            size_t secondChoice,
                            const HopoMatrix::ObsVector &observations) const {
  ...
  for (auto obs : knownObs[0])
    fstSet.insert(obs.id);
  std::unordered_set<uint32_t> commonSet;
  for (auto obs : knownObs[1]) {
    if (fstSet.count(obs.id))
      commonSet.insert(obs.id);
  }
  HopoMatrix::ObsVector commonObservations;
  for (auto obs : observations) {
    if (commonSet.count(obs.id))
      commonObservations.push_back(obs);
  }
