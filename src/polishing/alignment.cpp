...
Alignment::Alignment(size_t size, const SubstitutionMatrix &sm)
    : _forwardScores(size), _reverseScores(size), _subsMatrix(sm) {}
...
AlnScoreType Alignment::globalAlignment(const std::string &consensus,
  ...
  for (size_t readId = 0; readId < _forwardScores.size(); ++readId) {
    ...
    AlnScoreType score =
        this->getScoringMatrix(consensus, reads[readId], scoreMat);
...
AlnScoreType Alignment::addDeletion(unsigned int letterIndex) const {
  ...
  for (size_t readId = 0; readId < _forwardScores.size(); ++readId) {
    ...
    // Note: We subtract 2 because of zero indexing and an extra added row and
    // column count unsigned int index = (reverseScore.nrows() - 1) -
    // letterIndex;
    ...
    for (size_t col = 0; col < forwardScore.ncols(); ++col) {
      ...
      AlnScoreType sum =
          forwardScore.at(frontRow, col) + reverseScore.at(revRow, backCol);
...
//							   			unsigned
//int letterIndex, 							   			char base, const std::string& read)

AlnScoreType
Alignment::addSubstitution(unsigned int letterIndex, char base,
  ...
  for (size_t readId = 0; readId < reads.size(); ++readId) {
    ...
    for (size_t i = 0; i < reads[readId].size(); ++i) {
      ...
      AlnScoreType ins =
          forwardScore.at(frontRow, i + 1) + _subsMatrix.getScore(base, '-');
...
AlnScoreType
Alignment::addInsertion(unsigned int pos, char base,
                        const std::vector<std::string> &reads) const {
  AlnScoreType finalScore = 0;
  for (size_t readId = 0; readId < reads.size(); ++readId) {
    // LetterIndex must start with 1 and go until (row.size - 1)
...
AlnScoreType Alignment::getScoringMatrix(const std::string &v,
  ...
  for (size_t i = 1; i < v.size() + 1; i++) {
    ...
    for (size_t j = 1; j < w.size() + 1; j++) {
      ...
      AlnScoreType left =
          scoreMat.at(i, j - 1) + _subsMatrix.getScore('-', key2);
      AlnScoreType up = scoreMat.at(i - 1, j) + _subsMatrix.getScore(key1, '-');
      score = std::max(left, up);

      AlnScoreType cross =
          scoreMat.at(i - 1, j - 1) + _subsMatrix.getScore(key1, key2);
