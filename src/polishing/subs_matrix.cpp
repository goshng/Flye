...
size_t dnaToId(char c) { return table[(size_t)c]; }
struct TableFiller {
  TableFiller() {
...
AlnScoreType probToScore(double prob) {
  // std::cout << prob << " " << (int32_t)std::round(std::log(prob) *
  // (double)SCORE_MULT) << "\n";
...
HopoMatrix::State::State(const std::string &str, size_t start, size_t end) {
  ...
  for (size_t i = start; i < end; ++i) {
    if (str[i] != '-') {
      ...
      if (str[i] != runNucl)
        throw std::runtime_error("Wrong homopolymer");
      ++runLength;
    }
  }
  if (runLength == 0)
...
void HopoMatrix::loadMatrix(const std::string &fileName) {
  ...
  for (char nucl : std::string("ACGT")) {
    for (size_t runLen = MIN_HOPO; runLen <= MAX_HOPO; ++runLen) {
      ...
      // _genomeProbs[state.id]
      //		  << "\t" << sumFreq << "\t" <<
      //nucleotideFreq[dnaToId(state.nucl)]
      //		  << std::endl;
