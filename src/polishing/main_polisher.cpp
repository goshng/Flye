...
bool parseArgs(int argc, char **argv, std::string &bubblesFile,
  ...
  int optionIndex = 0;
  static option longOptions[] = {{"bubbles", required_argument, 0, 0},
  ...
  if (bubblesFile.empty() || scoringMatrix.empty() || hopoMatrix.empty() ||
      outConsensus.empty()) {
    printUsage();
    return false;
  }
...
int polisher_main(int argc, char *argv[]) {
  ...
  if (!parseArgs(argc, argv, bubblesFile, scoringMatrix, hopoMatrix,
                 outConsensus, outVerbose, numThreads, quiet, enableHopo))
    return 1;
