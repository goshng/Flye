...
template <class T> SetNode<T> *findSet(SetNode<T> *elem) {
  if (elem->parent != elem) {
...
template <class T> void unionSet(SetNode<T> *node1, SetNode<T> *node2) {
...
template <typename T> class SetVec : public std::vector<SetNode<T> *> {
public:
  ~SetVec() {
    for (auto &x : *this)
      delete x;
