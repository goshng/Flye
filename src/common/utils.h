...
template <class T> void vecRemove(std::vector<T> &v, T val) {
...
template <typename T> T quantile(const std::vector<T> &vec, int percent) {
  if (vec.empty())
    return 0;
  ...
  size_t targetId =
      std::min(vec.size() * (size_t)percent / 100, vec.size() - 1);
  return sortedVec[targetId];
}

template <typename T> T median(const std::vector<T> &vec) {
