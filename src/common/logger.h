...
public:
  ...
  public:
    StreamWriter(const std::string &level,
      ...
      if (_fileStream)
        *_fileStream << timestamp() << " " << level << " ";
      if (_consoleStream)
        *_consoleStream << timestamp() << " " << level << " ";
    }
    ~StreamWriter() {
      if (_fileStream)
        *_fileStream << std::endl;
      if (_consoleStream)
        *_consoleStream << std::endl;
    }

    template <class T> Logger::StreamWriter &operator<<(const T &val) {
      if (_fileStream)
        *_fileStream << val;
      if (_consoleStream)
        *_consoleStream << val;
...
private:
  ...
  Logger() : _debug(false), _logFileSet(false) {}
  ~Logger() {
    if (_logFileSet) {
