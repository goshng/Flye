...
public:
  KmerCounter(const SequenceContainer &seqContainer)
      : _seqContainer(seqContainer), _flatCounter(nullptr), _numKmers(0) {}
  ...
  const KmerDistribution &getKmerHist() const { return _kmerDistribution; }
...
class VertexIndex {
public:
  ~VertexIndex() { this->clear(); }
  VertexIndex(const SequenceContainer &seqContainer)
      : _seqContainer(seqContainer), _outputProgress(false), _sampleRate(1.0f),
        _repetitiveFrequency(0), _kmerCounter(seqContainer)
  //_solidMultiplier(1)
...
private:
  struct IndexChunk {
    IndexChunk() : hi(0), low(0) {}
    IndexChunk(const IndexChunk &other) : hi(other.hi), low(other.low) {}
    IndexChunk(IndexChunk &&other) : hi(other.hi), low(other.low) {}
    ...
    size_t get() const { return ((size_t)hi << 32) + (size_t)low; }
    void set(size_t val) {
      low = val & ((1ULL << 32) - 1);
      hi = val >> 32;
    }
...
public:
  ...
  public:
    KmerPosIterator(ReadVector rv, size_t index, bool revComp,
        ...
        : rv(rv), index(index), revComp(revComp), seqContainer(seqContainer),
  ...
  IterHelper iterKmerPos(Kmer kmer) const {
    ...
    return IterHelper(_kmerIndex.find(kmer), revComp, _seqContainer);
  }
...
private:
  ...
  std::vector<KmerFreq> yieldFrequentKmers(const FastaRecord::Id &seqId,
