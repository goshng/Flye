...
public:
  ...
  explicit DnaSequence(const std::string &string) : _complement(false) {
    _data = new SharedBuffer;
    ++_data->useCount;
  ...
  DnaSequence(const DnaSequence &other)
      : _data(other._data), _complement(other._complement) {
    ++_data->useCount;
  }
  ...
  char at(size_t index) const {
    ...
    size_t id =
        (_data->chunks[index / NUCL_IN_CHUNK] >> (index % NUCL_IN_CHUNK) * 2) &
        3;
  ...
  static size_t dnaToId(char c) { return _dnaTable[(size_t)c]; }
...
inline DnaSequence DnaSequence::substr(size_t start, size_t length) const {
  if (length == 0)
    throw std::runtime_error("Zero length subtring");
  if (start >= _data->length)
    throw std::runtime_error("Incorrect substring start");
