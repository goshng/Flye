...
struct ThreadMemPool {
  ThreadMemPool() : prevCleanup(system_clock::now() + seconds(rand() % 60)) {
    memPool = km_init();
  }
  ~ThreadMemPool() { km_destroy(memPool); }
  void cleanIter() {
...
CompressedSeq homopolymerCompression(const DnaSequence &seq, int32_t start,
                                     int32_t length, bool doCompression) {
  std::string newSeq;
...
float getAlignmentCigarKsw(const DnaSequence &trgSeq, size_t trgBegin,
                           size_t trgLen, const DnaSequence &qrySeq,
                           size_t qryBegin, size_t qryLen, float maxAlnErr,
  ...
  int8_t subsMat[] = {a, b, b, b, 0, b, a, b, b, 0, b, b, a,
  ...
  for (;;) {
    ...
    ksw_extz2_sse(buf.memPool, qryByte.size(), &qryByte[0], trgByte.size(),
                  &trgByte[0], NUM_NUCL, subsMat, gapOpen, gapExtend, bandWidth,
                  Z_DROP, END_BONUS, FLAG, &ez);
    if (!ez.zdropped) {
      ...
      {
        Logger::get().warning()
            << "Deviation: " << deviation << " " << bandWidth;
      }
      if (labs(deviation) <= bandWidth)
        break;
    }

    if (bandWidth > (int)std::max(qryByte.size(), trgByte.size()))
      break; // just in case
    bandWidth *= 2;
  }
  ...
  for (size_t i = 0; i < (size_t)ez.n_cigar; ++i) {
    ...
    if (op == 'M') {
      for (size_t i = 0; i < (size_t)size; ++i) {
        char match = "X="[size_t(trgByte[posTrg + i] == qryByte[posQry + i])];
...
float getAlignmentErrEdlib(const OverlapRange &ovlp, const DnaSequence &trgSeq,
                           const DnaSequence &qrySeq, float maxAlnErr,
                           bool useHpc) {
  thread_local ThreadMemPool buf;
  buf.cleanIter();

  auto trgCompressed =
      homopolymerCompression(trgSeq, ovlp.curBegin, ovlp.curRange(), useHpc);
  auto qryCompressed =
      homopolymerCompression(qrySeq, ovlp.extBegin, ovlp.extRange(), useHpc);
  ...
  auto result = edlibAlign(&qryCompressed.seq.str().c_str()[0],
                           qryCompressed.seq.length(),
                           &trgCompressed.seq.str().c_str()[0],
                           trgCompressed.seq.length(), edlibCfg);
  // Logger::get().debug() << result.editDistance << " " <<
  // result.alignmentLength;
  if (result.editDistance < 0) {
    return 1.0f;
  }
  return (float)result.editDistance /
         std::max(qryCompressed.seq.length(), trgCompressed.seq.length());
  // return (float)result.editDistance / result.alignmentLength;
}

float getAlignmentErrKsw(const OverlapRange &ovlp, const DnaSequence &trgSeq,
                         const DnaSequence &qrySeq, float maxAlnErr) {
...
void decodeCigar(const std::vector<CigOp> &cigar, const DnaSequence &trgSeq,
                 size_t trgBegin, const DnaSequence &qrySeq, size_t qryBegin,
...
std::vector<OverlapRange> checkIdyAndTrim(OverlapRange &ovlp,
                                          const DnaSequence &curSeq,
                                          const DnaSequence &extSeq,
                                          float maxDivergence,
                                          int32_t minOverlap, bool useHpc) {
  // homopolymer-compressed, if needed
  auto curCompressed =
      homopolymerCompression(curSeq, ovlp.curBegin, ovlp.curRange(), useHpc);
  auto extCompressed =
      homopolymerCompression(extSeq, ovlp.extBegin, ovlp.extRange(), useHpc);

  // recompute base alignment with cigar output
  std::vector<CigOp> cigar;
  float errRate = getAlignmentCigarKsw(
      curCompressed.seq, 0, curCompressed.seq.length(), extCompressed.seq, 0,
      extCompressed.seq.length(), maxDivergence, cigar);
  (void)errRate;
  ...
  for (int intLen = (int)cigar.size(); intLen > 0; --intLen) {
    for (int intStart = 0; intStart < (int)cigar.size() - intLen + 1;
         ++intStart) {
      int i = intStart;
      int j = intStart + intLen - 1;
      // only consider intervals that end on matches
      if (cigar[i].op != '=' || cigar[j].op != '=')
        continue;
      ...
      if (divergence < maxDivergence) {
        if (j - i >= 0)
          goodIntervals.push_back({i, j, divergence, rangeLen});
      }
    }
  }
  ...
  for (auto &interval : goodIntervals) {
    ...
    if (!intersects)
      nonIntersecting.push_back(interval);
  }
  ...
  for (auto intCand : nonIntersecting) {
    ...
    if (newOvlp.curRange() > minOverlap && newOvlp.extRange() > minOverlap) {
  ...
  if (!trimmedAlignments.empty())
  {
          ...
          float noHpcErr =
                  getAlignmentCigarKsw(curSeq, ovlp.curBegin, ovlp.curRange(),
                                                           extSeq,
  ovlp.extBegin, ovlp.extRange(), maxDivergence, cigar); decodeCigar(cigar,
  curSeq, ovlp.curBegin, extSeq, ovlp.extBegin, alnCur, alnExt);
          printAlignment(alnCur, alnExt);
          Logger::get().debug() << "Idy w/o hpc: " << noHpcErr << " with hpc: "
  << errRate;
          ...
          for (auto& newOvlp : trimmedAlignments)
          {
                  Logger::get().debug() << "      to cb:" << newOvlp.curBegin
                          << " cl:" << newOvlp.curRange() <<
                          " eb:" << newOvlp.extBegin << " el:" <<
  newOvlp.extRange()
                          << " err:" << newOvlp.seqDivergence
                          << " checkErr:" << getAlignmentErrEdlib(newOvlp,
  curSeq, extSeq, 1, false)
                          << " checkErrHpc:" << getAlignmentErrEdlib(newOvlp,
  curSeq, extSeq, 1, true);
          }
  }*/
