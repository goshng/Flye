...
FastaRecord::Id SequenceContainer::addSequence(const FastaRecord &seqRec) {
  ...
  _seqIndex.emplace_back(seqRec.sequence, "+" + seqRec.description, newId);
  ...
  _seqIndex.emplace_back(seqRec.sequence.complement(), "-" + seqRec.description,
                         newId.rc());
  _nameIndex[_seqIndex.back().description] = _seqIndex.back().id;
...
SequenceContainer::addSequence(const DnaSequence &sequence,
  ...
  auto newId = this->addSequence({sequence, description, FastaRecord::ID_NONE});
  ...
  try {
    while (!gzeof(fd)) {
      ...
      if (nextLine[0] == '>') {
        if (!header.empty()) {
          if (sequence.empty())
            throw ParseException("empty sequence");
    ...
    record.emplace_back(DnaSequence(sequence), header, FastaRecord::ID_NONE);

  } catch (ParseException &e) {
    std::stringstream ss;
    ss << "parse error in " << fileName << " on line " << lineNo << ": "
       << e.what();
    gzclose(fd);
...
size_t SequenceContainer::readFastq(std::vector<FastaRecord> &record,
  ...
  try {
    while (!gzeof(fd)) {
      ...
      if (stateCounter == 0) {
        if (nextLine[0] != '@')
          throw ParseException("Fastq format error");
        header = nextLine;
        ...
        if (nextLine[0] != '+')
          throw ParseException("Fastq fromat error");
      }
      stateCounter = (stateCounter + 1) % 4;
      ++lineNo;
      nextLine.clear();
...
void SequenceContainer::validateHeader(std::string &header) {
  ...
  for (delim = 0; delim < header.length(); ++delim) {
    if (std::isspace(header[delim]))
      break;
  }

  header = header.substr(1, delim - 1);
  if (header.empty())
    throw ParseException("empty header");
}
...
void SequenceContainer::writeFasta(const std::vector<FastaRecord> &records,
  ...
  for (const auto &rec : records) {
    if (onlyPositiveStrand && !rec.id.strand())
      continue;
    ...
    fwrite(contigSeq.data(), sizeof(contigSeq.data()[0]), contigSeq.size(),
           fout);
  }
  fclose(fout);
}
...
void SequenceContainer::buildPositionIndex() {
  ...
  for (size_t i = 0; i <= (offset - 1) / CHUNK; ++i) {
    while (i * CHUNK >= _sequenceOffsets[idx + 1].offset)
      ++idx;
    // size_t newIdx = std::upper_bound(_sequenceOffsets.begin(),
    //							     _sequenceOffsets.end(),
    //							     i * CHUNK) -
    //_sequenceOffsets.begin(); Logger::get().debug() << idx << " " << newIdx;
    // assert(idx == newIdx);
