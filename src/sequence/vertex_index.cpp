...
void VertexIndex::buildIndexUnevenCoverage(int globalMinFreq, float selectRate,
  ...
  if (_outputProgress)
    Logger::get().info() << "Filling index table (1/2)";
  std::function<void(const FastaRecord::Id &)> initializeIndex =
      [this, globalMinFreq, selectRate,
       tandemFreq](const FastaRecord::Id &readId) {
        if (!readId.strand())
          return;

        auto topKmers =
            this->yieldFrequentKmers(readId, selectRate, tandemFreq);
        for (auto kmerFreq : topKmers) {
          ...
          _kmerIndex.upsert(
              kmerFreq.kmer, [](ReadVector &rv) { ++rv.capacity; }, defVec);
        }
      };
  processInParallel(allReads, initializeIndex, Parameters::get().numThreads,
                    _outputProgress);

  this->filterFrequentKmers(globalMinFreq,
                            (float)Config::get("repeat_kmer_rate"));
  this->allocateIndexMemory();

  if (_outputProgress)
    Logger::get().info() << "Filling index table (2/2)";
  std::function<void(const FastaRecord::Id &)> indexUpdate =
      [this, globalMinFreq, selectRate,
       tandemFreq](const FastaRecord::Id &readId) {
        ...
        for (auto kmerFreq : topKmers) {
          if (kmerFreq.freq < (size_t)globalMinFreq ||
              kmerFreq.freq > _repetitiveFrequency)
            continue;
          ...
          if (revCmp) {
            kmerPos.position = _seqContainer.seqLen(readId) - kmerPos.position -
                               Parameters::get().kmerSize;
            targetRead = targetRead.rc();
          }

          // will not trigger update for k-mer not in the index
          _kmerIndex.update_fn(kmerPos.kmer, [targetRead, &kmerPos,
                                              this](ReadVector &rv) {
            ...
            size_t globPos =
                _seqContainer.globalPosition(targetRead, kmerPos.position);
            rv.data[rv.size].set(globPos);
            ++rv.size;
          });
        }
      };
  processInParallel(allReads, indexUpdate, Parameters::get().numThreads,
                    _outputProgress);
...
namespace {
template <class T> size_t getFreq(T &histIter) { return histIter->first; };

template <class T> size_t getCount(T &histIter) { return histIter->second; };
...
void VertexIndex::filterFrequentKmers(int minCoverage, float rate) {
  ...
  Logger::get().debug() << "Mean k-mer frequency: " << meanFrequency;
  Logger::get().debug() << "Repetitive k-mer frequency: "
                        << _repetitiveFrequency;
  Logger::get().debug() << "Filtered " << repetitiveKmers
                        << " repetitive k-mers (" << filteredRate << ")";
}

std::vector<VertexIndex::KmerFreq>
VertexIndex::yieldFrequentKmers(const FastaRecord::Id &seqId, float selectRate,
                                int tandemFreq) {
  ...
  if (topKmers.empty())
    return {};
  std::sort(
      topKmers.begin(), topKmers.end(),
      [](const KmerFreq &k1, const KmerFreq &k2) { return k1.freq > k2.freq; });
  const size_t maxKmers = selectRate * topKmers.size();
  const size_t minFreq = topKmers[maxKmers].freq;

  auto itVec = topKmers.begin();
  while (itVec != topKmers.end() && itVec->freq >= minFreq)
    ++itVec;
...
void VertexIndex::buildIndexMinimizers(int minCoverage, int wndLen) {
  if (_outputProgress)
    Logger::get().info() << "Building minimizer index";
  ...
  for (const auto &seq : _seqContainer.iterSeqs()) {
    allReads.push_back(seq.id);
    if (seq.id.strand())
      totalLen += seq.sequence.length();
  }

  _kmerIndex.reserve(1000000);
  if (_outputProgress)
    Logger::get().info() << "Pre-calculating index storage";
  std::function<void(const FastaRecord::Id &)> initializeIndex =
      [this, minCoverage, wndLen](const FastaRecord::Id &readId) {
        ...
        for (auto kmerPos : minimizers) {
          ...
          ReadVector defVec((uint32_t)1, (uint32_t)0);
          _kmerIndex.upsert(
              stdKmer, [](ReadVector &rv) { ++rv.capacity; }, defVec);
        }
      };
  processInParallel(allReads, initializeIndex, Parameters::get().numThreads,
                    _outputProgress);

  this->filterFrequentKmers(minCoverage,
                            (float)Config::get("repeat_kmer_rate"));
  this->allocateIndexMemory();

  if (_outputProgress)
    Logger::get().info() << "Filling index";
  std::function<void(const FastaRecord::Id &)> indexUpdate =
      [this, minCoverage, wndLen](const FastaRecord::Id &readId) {
        if (!readId.strand())
          return;
...
void KmerCounter::count(bool useFlatCounter) {
  ...
  if (_outputProgress)
    Logger::get().info() << "Counting k-mers:";
        ...
        for (auto kmerPos : IterKmers(_seqContainer.getSeq(readId))) {
          ...
          if (_useFlatCounter) {
            ...
            while (true) {
              ...
              if (_flatCounter[arrayPos].compare_exchange_weak(expected,
                                                               updated)) {
                if (count == 0)
                  ++_numKmers;
  ...
  processInParallel(allReads, readUpdate, Parameters::get().numThreads,
                    _outputProgress);
  ...
  if (_useFlatCounter) {
    for (size_t kmerId = 0; kmerId < COUNTER_LEN * 2; ++kmerId) {
      ...
      if (freq > 0)
        _kmerDistribution[freq] += 1;
      // if (kmerId % 1000000 == 0) Logger::get().debug() << kmerId << " " <<
      // freq;
    }
  } else {
...
size_t KmerCounter::getFreq(Kmer kmer) const {
  ...
  if (_useFlatCounter) {
    ...
    uint8_t count = highBits ? (_flatCounter[arrayPos]) >> 4
                             : (_flatCounter[arrayPos] & 15);
    if (count < 15) {
      return count;
    } else {
      addCount = count;
