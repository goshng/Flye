...
struct OverlapRange {
  ...
  int32_t project(int32_t curPos) const {
    ...
    if (!kmerMatches) {
      ...
      int32_t projectedPos = extBegin + float(curPos - curBegin) * lengthRatio;
      return std::max(extBegin, std::min(projectedPos, extEnd));
    } else {
      auto cmpFirst = [](const std::pair<int32_t, int32_t> &pair,
                         int32_t value) { return pair.first < value; };
      ...
      int32_t curInt = (*kmerMatches)[i].first - (*kmerMatches)[i - 1].first;
      int32_t extInt = (*kmerMatches)[i].second - (*kmerMatches)[i - 1].second;
      float lengthRatio = (float)extInt / curInt;
      int32_t projectedPos =
  ...
  int32_t leftShift() const { return curBegin - extBegin; }

  int32_t rightShift() const { return (extLen - extEnd) - (curLen - curEnd); }
  ...
  bool contains(int32_t curPos, int32_t extPos) const {
    return curBegin <= curPos && curPos <= curEnd && extBegin <= extPos &&
           extPos <= extEnd;
  }

  bool containedBy(const OverlapRange &other) const {
    if (curId != other.curId || extId != other.extId)
      return false;
  ...
  int32_t curIntersect(const OverlapRange &other) const {
    return std::min(curEnd, other.curEnd) - std::max(curBegin, other.curBegin);
  }
...
private:
  std::vector<OverlapRange> getSeqOverlaps(const FastaRecord &fastaRec,
                                           bool forceLocal,
...
public:
  ...
  struct IndexVecWrapper {
    IndexVecWrapper()
        : fwdOverlaps(new std::vector<OverlapRange>),
          revOverlaps(new std::vector<OverlapRange>), cached(false),
  ...
  getCoveringOverlaps(FastaRecord::Id seqId, int32_t start, int32_t end) const;
...
private:
  ...
  std::unordered_map<FastaRecord::Id, IntervalTree<const OverlapRange *>>
      _ovlpTree;
...
public:
  OvlpIterator(std::vector<OverlapRange>::const_iterator it,
    ...
    if (onlyNoOverhang) {
      ...
      while (it != end && it->lrOverhang() > MAX_OVERHANG)
        ++it;
    }
  }
  ...
  bool operator!=(const OvlpIterator &other) const { return !(*this == other); }

  //__attribute__((always_inline))
  const OverlapRange &operator*() const { return *it; }
