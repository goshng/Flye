...
bool OverlapDetector::overlapTest(const OverlapRange &ovlp,
  ...
  if (ovlp.curRange() < _minOverlap || ovlp.extRange() < _minOverlap) {
    return false;
  }
  ...
  if (!forceLocal && _checkOverhang && ovlp.lrOverhang() > _maxOverhang)
    return false;
...
template <class T> void shrinkAndClear(std::vector<T> &vec, float rate) {
  if (vec.empty())
    return;
...
OverlapDetector::getSeqOverlaps(const FastaRecord &fastaRec, bool forceLocal,
                                OvlpDivStats &divStats, int maxOverlaps) const {
  ...
  for (const auto &curKmerPos : IterKmers(fastaRec.sequence)) {
    ...
    if (!_vertexIndex.kmerFreq(curKmerPos.kmer))
      continue;
    ...
    for (const auto &extReadPos : _vertexIndex.iterKmerPos(curKmerPos.kmer)) {
      ...
      vecMatches.emplace_back(curKmerPos.position, extReadPos.position,
                              extReadPos.readId);
    }
  }
  timeKmerIndexFirst +=
  ...
  while (extRangeEnd < vecMatches.size()) {
    if (maxOverlaps != 0 && detectedOverlaps.size() >= (size_t)maxOverlaps)
      break;
    ...
    int32_t prevPos = 0;
    while (extRangeEnd < vecMatches.size() &&
           vecMatches[extRangeBegin].extId == vecMatches[extRangeEnd].extId) {
    ...
    if (uniqueMatches < minKmerSruvivalRate * _minOverlap)
      continue;
    ...
    if (maxCur - minCur < _minOverlap || maxExt - minExt < _minOverlap)
      continue;
    if (_checkOverhang && !forceLocal) {
      if (std::min(minCur, minExt) > _maxOverhang)
        continue;
      if (std::min(curLen - maxCur, extLen - maxExt) > _maxOverhang)
        continue;
    }
    //++uniqueCandidates;
    ...
    for (int32_t i = 1; i < (int32_t)scoreTable.size(); ++i) {
      ...
      for (int32_t j = i - 1; j >= 0; --j) {
        ...
        int32_t jumpDiv = abs((curNext - curPrev) - (extNext - extPrev));
        if (0 < curNext - curPrev && curNext - curPrev < _maxJump &&
            0 < extNext - extPrev && extNext - extPrev < _maxJump &&
            jumpDiv <= MAX_GAP) {
          int32_t matchScore = std::min(
              std::min(curNext - curPrev, extNext - extPrev), kmerSize);
          // int32_t gapCost = jumpDiv ?
          //		kmerSize * jumpDiv + ilog2_32(jumpDiv) : 0;
          int32_t gapCost =
              (jumpDiv > GAP_JUMP_THLD ? LG_GAP : SM_GAP) * jumpDiv;
          ...
          if (nextScore > maxScore) {
            ...
            if (jumpDiv == 0 && curNext - curPrev < kmerSize)
              break;
          }
          /*else
          {
        ...
        if (extSorted && extNext - extPrev > _maxJump)
          break;
        if (!extSorted && curNext - curPrev > _maxJump)
          break;
      }
    ...
    // initiate chains from the highest scores in the table (e.g. local
    // maximums)
    ...
    for (int32_t chainStart : orderedScores) {
      if (backtrackTable[chainStart] == -1)
        continue;
      ...
      while (pos != -1) {
        ...
        if (_keepAlignment) {
          if (kmerMatches.empty() ||
              kmerMatches.back().first - matchesList[pos].curPos > kmerSize) {
      ...
      // Logger::get().debug() << chainStart - firstMatch << " " << lastMatch -
      // firstMatch;

      OverlapRange ovlp(fastaRec.id, matchesList.front().extId,
                        matchesList[firstMatch].curPos,
                        matchesList[firstMatch].extPos, curLen, extLen);
      ...
      if (this->overlapTest(ovlp, forceLocal)) {
        ...
        float normLen =
            std::max(ovlp.curRange(), ovlp.extRange()) - filteredPositions;
        float matchRate =
            (float)chainLength * _vertexIndex.getSampleRate() / normLen;
    ...
    {
      if (!extOverlaps.empty())
        primaryOverlaps.push_back(extOverlaps.front());
    } else {
    ...
    for (auto &ovlp : primaryOverlaps) {
      ...
      // if alignment not passing thrshold, check if its parts do
        ...
        auto trimmedOverlaps = checkIdyAndTrim(
...
OverlapContainer::quickSeqOverlaps(FastaRecord::Id readId, int maxOverlaps,
  ...
  return _ovlpDetect.getSeqOverlaps(record, forceLocal, _divergenceStats,
                                    maxOverlaps);
}

std::vector<OverlapRange>
OverlapContainer::quickSeqOverlaps(const FastaRecord &record, int maxOverlaps,
                                   bool forceLocal) {
  return _ovlpDetect.getSeqOverlaps(record, forceLocal, _divergenceStats,
                                    maxOverlaps);
}
...
OverlapContainer::lazySeqOverlaps(FastaRecord::Id readId) {
  ...
  _overlapIndex.upsert(readId,
                       [&wrapper](IndexVecWrapper &val) { wrapper = val; });
  if (wrapper.cached) {
  ...
  _overlapIndex.update_fn(
      readId, [&wrapper, &overlaps, &revOverlaps, this](IndexVecWrapper &val) {
        if (!val.cached) {
...
void OverlapContainer::findAllOverlaps() {
  ...
  processInParallel(allQueries, indexUpdate, Parameters::get().numThreads,
                    true);
...
OverlapContainer::unsafeSeqOverlaps(FastaRecord::Id seqId) {
  ...
  return seqId.strand() ? *wrapper.fwdOverlaps : *wrapper.revOverlaps;
}
...
void OverlapContainer::filterOverlaps() {
      ...
      [this](const FastaRecord::Id &seqId) {
        ...
        for (size_t i = 0; i < overlapSets.size(); ++i) {
          for (size_t j = 0; j < overlapSets.size(); ++j) {
            ...
            if (ovlpOne.extId != ovlpTwo.extId)
              continue;
  ...
  processInParallel(seqIds, filterParallel, Parameters::get().numThreads,
                    false);
}
...
void OverlapContainer::estimateOverlaperParameters() {
  ...
  std::function<void(const FastaRecord::Id &seqId)> computeParallel =
      [this, &storageMutex, &biases,
       &trueDivergence](const FastaRecord::Id &seqId) {
        ...
        {
                float trueDiv =
                        getAlignmentErrEdlib(ovlp,
        _queryContainer.getSeq(seqId),
  ...
  processInParallel(readsToCheck, computeParallel, Parameters::get().numThreads,
                    false);
  ...
  if (!trueDivergence.empty()) {
    ...
    Logger::get().warning()
        << "No overlaps found - unable to estimate parameters";
    _meanTrueOvlpDiv = 0.5f;
    //_kmerIdyEstimateBias = 0.0f;
  }

  Logger::get().debug() << "Initial divergence estimate : " << _meanTrueOvlpDiv;
  // Logger::get().debug() << "K-mer estimate bias (true - est): " <<
  // _kmerIdyEstimateBias;
}

void OverlapContainer::setDivergenceThreshold(float threshold,
                                              bool isRelative) {

  _ovlpDetect._maxDivergence =
      (isRelative ? _meanTrueOvlpDiv : 0.0f) + threshold;
...
void OverlapContainer::overlapDivergenceStats() {
  this->overlapDivergenceStats(_divergenceStats, _ovlpDetect._maxDivergence);
}
...
void OverlapContainer::overlapDivergenceStats(const OvlpDivStats &stats,
  ...
  Logger::get().debug() << "Sequence divergence distribution: \n"
                        ...
                        << quantile(ovlpDivergence, 25)
                        << ", Q50 = " << quantile(ovlpDivergence, 50)
...
OverlapContainer::getCoveringOverlaps(FastaRecord::Id seqId, int32_t start,
                                      int32_t end) const {
