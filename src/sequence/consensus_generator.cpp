...
ConsensusGenerator::generateConsensuses(const std::vector<ContigPath> &contigs,
                                        bool verbose) {
  if (verbose)
    Logger::get().info() << "Generating sequence";
...
FastaRecord ConsensusGenerator::generateLinear(const ContigPath &path,
  ...
  for (size_t i = 0; i < path.sequences.size(); ++i) {
    ...
    if (i != path.sequences.size() - 1) {
      auto curSwitch = this->getSwitchPositions(alnMap.at(&path.overlaps[i]),
                                                prevSwitch.second);
...
ConsensusGenerator::generateAlignments(const std::vector<ContigPath> &contigs,
  ...
  std::mutex mapMutex;
  std::function<void(const AlnTask &)> alnFunc = [&alnMap, &mapMutex](
                                                     const AlnTask &task) {
    ...
    getAlignmentCigarKsw(path->sequences[i], curOverlap.curBegin,
                         curOverlap.curRange(), path->sequences[i + 1],
                         curOverlap.extBegin, curOverlap.extRange(), maxErr,
                         cigar);
    decodeCigar(cigar, path->sequences[i], curOverlap.curBegin,
                path->sequences[i + 1], curOverlap.extBegin, alignedLeft,
                alignedRight);

    {
      std::lock_guard<std::mutex> lock(mapMutex);
      alnMap[&path->overlaps[i]] = {alignedLeft, alignedRight,
                                    curOverlap.curBegin, curOverlap.extBegin};
    }
  };
  ...
  for (auto &path : contigs) {
    ...
    for (size_t i = 0; i < path.sequences.size() - 1; ++i) {
      ...
      int32_t endShift =
          std::min(curOverlap.curRange(), curOverlap.extRange()) -
          MAX_ALIGNMENT;
      if (endShift > 0) {
