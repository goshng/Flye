...
struct FastaRecord {
  ...
  FastaRecord(const FastaRecord &other)
      : id(other.id), sequence(other.sequence), description(other.description) {
  }

  FastaRecord(FastaRecord &&other) : id(other.id) { *this = std::move(other); }
...
namespace std {
template <> struct hash<FastaRecord::Id> {
  size_t operator()(const FastaRecord::Id &h) const throw() { return h.hash(); }
};

template <> struct hash<FastaRecord::IdPair> {
...
public:
  ...
  SequenceContainer() : _offsetInitialized(false) {}
  ...
  const SequenceIndex &iterSeqs() const { return _seqIndex; }
...
#ifndef NDEBUG
    auto checkGlob =
        _sequenceOffsets[seqId._id - _seqIdOffest].offset + position;
  ...
  void seqPosition(size_t globPos, FastaRecord::Id &outSeqId,
    ...
    while (_sequenceOffsets[hint + 1].offset <= globPos)
      ++hint;
...
private:
  ...
  std::unordered_map<std::string, FastaRecord::Id> _nameIndex;
