...
size_t ParametersEstimator::genomeSizeEstimate() { return _takenKmers; }
...
void ParametersEstimator::estimateMinKmerCount() {
  ...
  for (auto mapPair = _vertexIndex.getKmerHist().rbegin();
    ...
    prevDiff =
        std::max(takenKmers, _genomeSize) - std::min(takenKmers, _genomeSize);
  }

  size_t filteredKmers = 0;
  for (auto itKmer : _vertexIndex.getKmerHist()) {
    if (itKmer.first >= cutoff)
      break;
  ...
  if (cutoff < 2) {
    if ((bool)Config::get("low_cutoff_warning")) {
      Logger::get().warning()
          << "Unable to separate erroneous k-mers "
