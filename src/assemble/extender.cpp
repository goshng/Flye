...
Extender::ExtensionInfo Extender::extendDisjointig(FastaRecord::Id startRead) {
  ...
  auto leftExtendsStart = [startRead, this,
                           &startOverlaps](const FastaRecord::Id readId) {
    for (const auto &ovlp : IterNoOverhang(startOverlaps)) {
      if (ovlp.extId == readId && this->extendsLeft(ovlp))
        return true;
    }
    return false;
  };
  ...
  while (true) {
    ...
    for (auto& ovlp : overlaps)
    {
            if (this->extendsRight(ovlp))
            {
                    Logger::get().debug() << "\t" <<
                            ...
                            << "\text:" <<
    this->countRightExtensions(ovlp.extId);
            }
    }*/
    ...
    for (const auto &ovlp : extensions) {
      ...
      if (leftExtendsStart(ovlp.extId))
        continue;

      // only using reads longer than safeOverlap
      if (ovlp.extLen < _safeOverlap)
        continue;
      ...
      if (ovlp.minRange() < _safeOverlap) {
        bool curRepeat = _chimDetector.isRepetitiveRegion(
            ovlp.curId, ovlp.curBegin, ovlp.curEnd);
        bool extRepeat = _chimDetector.isRepetitiveRegion(
            ovlp.extId, ovlp.extBegin, ovlp.extEnd);
        if (curRepeat && extRepeat)
          continue;
      }

      const std::vector<OverlapRange> &extOverlaps =
          _ovlpContainer.lazySeqOverlaps(ovlp.extId);

      const float MAX_COVERAGE_DROP = 5.0f;
      if (_chimDetector.isChimeric(ovlp.extId, extOverlaps) &&
          _chimDetector.maxCoverageDrop(ovlp.extId, extOverlaps) >
              MAX_COVERAGE_DROP)
        continue;

      // optimistically, pick the first available highly reliable extenion
      // (which will also be with the longest overlap as extensions are sorted
      // based on that)
      if (!_chimDetector.isChimeric(ovlp.extId, extOverlaps) &&
    ...
    if (selectedExtension) {
      ...
      if (selectedExtension->minRange() < _safeOverlap)
        ++exInfo.shortExtensions;
  ...
  if (!overlapSizes.empty()) {
    ...
    exInfo.minOverlapSize =
        *std::min_element(overlapSizes.begin(), overlapSizes.end());
  }
...
void Extender::assembleDisjointigs() {
  ...
  auto processRead = [this, &indexMutex, &coveredReads, totalReads,
    ...
    if (aggressiveDupFilt && numInnerOvlp > totalOverlaps / 2)
      return;
    ...
    {
            //Logger::get().debug() << "Thrown away: " << exInfo.reads.size() <<
    " " << exInfo.numSuspicious
    ...
    for (size_t i = 1; i < exInfo.reads.size() - 1; ++i) {
      if (_innerReads.contains(exInfo.reads[i]))
        ++innerCount;
    }
    int innerThreshold = std::min(
        (int)Config::get("max_inner_reads"),
        int((float)Config::get("max_inner_fraction") * exInfo.reads.size()));
    if (innerCount > innerThreshold) {
      Logger::get().debug()
          << "Discarded disjointig with " << exInfo.reads.size()
          << " reads and " << innerCount << " inner overlaps";
      return;
    }
    ...
    Logger::get().debug() << "Inner: " << _innerReads.size()
                          << " covered: " << coveredReads.size()
                          << " total: " << totalReads;
  ...
  std::function<void(const FastaRecord::Id &)> threadWorker =
      [processRead](const FastaRecord::Id &readId) { processRead(readId); };
  ...
  processInParallel(allReads, threadWorker, Parameters::get().numThreads,
                    /*progress*/ false);
  progress.setDone();
  ...
  IterNoOverhang(_ovlpContainer.lazySeqOverlaps(readId)))
                          {
                                  if (ovlp.leftShift() >= 0 && ovlp.rightShift()
  <= 0)
                                  {
          ...
          Logger::get().info() << "Added " << singletonsAdded << " singleton
  reads";
  }*/
...
Extender::getInnerReads(const std::vector<OverlapRange> &ovlps) {
  ...
  for (const auto &ovlp : ovlps) {
    ...
    for (int pos = ovlp.extBegin / WINDOW + 1; pos < ovlp.extEnd / WINDOW;
         ++pos) {
      ++coverage[pos];
    }
  }
  ...
  for (auto rc : readsCoverage) {
    ...
    int rightZeros = 0;
    for (size_t i = 0; i < rc.second.size(); ++i) {
      if (rc.second[rc.second.size() - i - 1] != 0)
        break;
      ++rightZeros;
    }
    ...
    if (!middleZero && leftZeros < OVERHANG / WINDOW &&
        rightZeros < OVERHANG / WINDOW)
      innerReads.push_back(rc.first);
...
int Extender::countRightExtensions(
...
{
        return
this->countRightExtensions(_ovlpContainer.lazySeqOverlaps(readId));
}*/
...
int Extender::countLeftExtensions(
    const std::vector<OverlapRange> &ovlps) const {
  int count = 0;
  for (const auto &ovlp : IterNoOverhang(ovlps)) {
