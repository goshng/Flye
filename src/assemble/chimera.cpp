...
std::vector<int32_t> ChimeraDetector::getReadCoverage(
    FastaRecord::Id readId, const std::vector<OverlapRange> &readOverlaps) {
...
float ChimeraDetector::maxCoverageDrop(
  ...
  for (int32_t i = goodStart; i <= goodEnd - 1; ++i) {
    float rate = (float)(std::max(coverage[i], coverage[i + 1]) + 1) /
                 (std::min(coverage[i], coverage[i + 1]) + 1);
    if (maxDrop < rate)
      maxDrop = rate;
  }
...
bool ChimeraDetector::testReadByCoverage(
    FastaRecord::Id readId, const std::vector<OverlapRange> &readOvlps) {
  ...
  if (!Parameters::get().unevenCoverage) {
    threshold =
        std::max(1L, std::lround((float)_overlapCoverage / MAX_DROP_RATE));
  } else {
    threshold = std::min(
        10L, std::max(1L, std::lround(medianCoverage / MAX_DROP_RATE)));
  }
  ...
  fout << "max: " << maxCov << " median: " << medianCoverage
          << " threshold: " << threshold << " chim:" << lowCoverage << std::endl
  << std::endl; logLock.unlock();*/
...
bool ChimeraDetector::isRepetitiveRegion(FastaRecord::Id readId, int32_t start,
  ...
  /*int numWindows = std::ceil((float)_seqContainer.seqLen(readId) / WINDOW) +
  1; int vecSize = numWindows - 2 * FLANK; if (vecSize <= 0) return false;
  ...
  for (int32_t pos = std::max(0, start / WINDOW);
    ...
    if (HANG_END_RATE * coverage[pos] <=
        junctions[pos]) // if both are 0, it's suspicious
    {
      ++numSuspicious;
    }
    ++rangeLen;
  ...
  if (debug) {
    ...
    Logger::get().debug() << _seqContainer.seqLen(readId) << " " << start << " "
                          << end;
...
ChimeraDetector::getCachedCoverage(FastaRecord::Id readId) {
  ...
  _localOvlpsStorage.upsert(
      readId, [&cached](CachedCoverage &val) { cached = val; },
      CachedCoverage());
  if (cached.cached) {
    return cached;
  }
  ...
  if (vecSize <= 0)
    throw std::runtime_error("Zero-sized coverage vector");
  ...
  // updating cache
  _localOvlpsStorage.update_fn(
      readId, [&cached, &coverage, &junctions, this](CachedCoverage &val) {
        if (!val.cached) {
