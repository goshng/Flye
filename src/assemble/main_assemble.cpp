...
bool parseArgs(int argc, char **argv, std::string &readsFasta,
               std::string &outAssembly, std::string &logFile,
  ...
  auto printUsage = []() {
    std::cerr
        ...
        << "[default = false] \n"
        << "  --short \t\tassemble short sequences at a cost of possibly "
           "reduced contiguity "
        << "[default = false] \n"
  ...
  if (readsFasta.empty() || outAssembly.empty() || configPath.empty()) {
    printUsage();
    return false;
  }
...
void removeContainedDisjointigs(std::vector<FastaRecord> &disjointigs,
  ...
  OverlapDetector ovlp(
      disjSequences, vertIndex, (int)Config::get("maximum_jump"),
  ...
  for (auto &seq : disjSequences.iterSeqs()) {
    for (auto &ovlp : disjOverlaps.lazySeqOverlaps(seq.id)) {
      ...
      if (contained) {
        FastaRecord::Id contId =
            ovlp.curLen < ovlp.extLen ? ovlp.curId : ovlp.extId;
...
int assemble_main(int argc, char **argv) {
  ...
  Logger::get().debug() << "Total CPUs: "
                        << std::thread::hardware_concurrency();

  Config::load(configPath);
  if (!extraParams.empty())
    Config::addParameters(extraParams);
  if (kmerSize == -1) {
  ...
  OverlapDetector ovlp(
      readsContainer, vertexIndex, (int)Config::get("maximum_jump"),
      Parameters::get().minimumOverlap, (int)Config::get("maximum_overhang"),
      /*store alignment*/ false,
      /*only max ovlp*/ true,
  ...
  //{
  removeContainedDisjointigs(disjointigsFasta,
                             readOverlaps.getDivergenceThreshold());
  //}
