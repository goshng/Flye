...
bool parseArgs(int argc, char **argv, std::string &readsFasta,
  ...
  int optionIndex = 0;
  static option longOptions[] = {{"graph-edges", required_argument, 0, 0},
  ...
  if (readsFasta.empty() || outFolder.empty() || inGraphEdges.empty() ||
      configPath.empty() || inRepeatGraph.empty() || inReadsAlignment.empty()) {
    printUsage();
    return false;
  }
