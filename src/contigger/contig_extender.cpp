...
void ContigExtender::generateUnbranchingPaths() {
  ...
  for (auto &path : _unbranchingPaths) {
    ...
    for (auto &edge : path.path) {
      if (edge->altGroupId != -1 && edge->altGroupId % 2 == 0)
        ++numEven;
    }
...
void ContigExtender::generateContigs() {
  ...
  std::unordered_map<GraphEdge *, std::vector<const GraphAlignment *>> alnIndex;
  ...
  auto canTraverse = [&repeatDirections](const GraphEdge *edge) {
    ...
    return !repeatDirections.count(edge) || repeatDirections.at(edge);
  };

  typedef std::pair<GraphPath, std::string> PathAndSeq;
  auto extendPathRight = [this, &coveredRepeats, &repeatDirections, &upathsSeqs,
                          &canTraverse, &alnIndex,
                          graphContinue](UnbranchingPath &upath) {
    ...
    for (auto pathPtr : alnIndex[upath.path.back()]) {
      ...
      for (size_t i = 0; i < path.size(); ++i) {
        if (path[i].edge == upath.path.back() && i < path.size() - 1) {
          size_t j = i + 1;
          while (j < path.size() && path[j].edge->repetitive &&
                 !path[j].edge->altHaplotype && canTraverse(path[j].edge))
            ++j;
          if (j == i + 1)
            break;

          int32_t alnLen =
              path[j - 1].overlap.curEnd - path[i + 1].overlap.curBegin;
    ...
    for (size_t i = 0; i < upathAln.size(); ++i) {
      ...
      // incomplete
      if (i == upathAln.size() - 1 && lastIncomplete && !graphContinue)
        break;
    ...
    if (!upathAln.empty()) {
      ...
      extendedSeq =
          _readSeqs.getSeq(readId).substr(readStart, readEnd - readStart).str();
    }
    if (lastIncomplete && graphContinue) {
    ...
    for (auto &ualn : upathAln) {
      for (auto &edgeAln : ualn.aln)
        extendedPath.push_back(edgeAln.edge);
    }
    if (lastIncomplete && graphContinue) {
      for (auto &edge : lastUpath->path)
        extendedPath.push_back(edge);
    }
  ...
  for (auto &upath : _unbranchingPaths) {
    if (upath.repetitive || !upath.id.strand())
      continue;
    if (!idToPath.count(upath.id.rc()))
      continue; // self-complement
    ...
    for (auto &path : leftPaths) {
      leftEdges.insert(leftEdges.end(), path->path.begin(), path->path.end());
    }
    contig.graphEdges.path.insert(contig.graphEdges.path.begin(),
                                  leftEdges.begin(), leftEdges.end());
    contig.graphPaths.insert(contig.graphPaths.begin(), leftPaths.begin(),
                             leftPaths.end());

    GraphPath rightEdges;
    for (auto &path : rightPaths) {
      rightEdges.insert(rightEdges.end(), path->path.begin(), path->path.end());
    }
    contig.graphEdges.path.insert(contig.graphEdges.path.end(),
                                  rightEdges.begin(), rightEdges.end());
    contig.graphPaths.insert(contig.graphPaths.end(), rightPaths.begin(),
                             rightPaths.end());
...
void ContigExtender::outputStatsTable(const std::string &filename) {
  ...
  for (auto &ctg : _contigs) {
    ...
    int estMult = std::max(
        1.0f, std::round((float)ctg.graphEdges.meanCoverage / meanCoverage));
    ...
    fout << ctg.graphEdges.name() << "\t" << ctg.sequence.length() << "\t"
         ...
         << YES_NO[ctg.graphEdges.repetitive] << "\t" << estMult << "\t"
         << telomereStr << "\t" << altGroupStr << "\t" << pathStr << "\n";
...
void ContigExtender::outputScaffoldConnections(const std::string &filename) {
  ...
  for (auto &edge : _graph.iterEdges()) {
    ...
    if (reachableEdges(_graph.complementEdge(outEdge)).size() != 1)
      continue;
