...
class RgEdge(object):
    ...
    def length(self):
        ...
        return sum([s.edge_seq_len for s in self.edge_sequences]) // len(
            self.edge_sequences
        )

    def __repr__(self):
        return "(id={0}, len={1}, cov={2} rep={3})".format(
            self.edge_id, self.length(), self.mean_coverage, self.repetitive
        )
...
class RepeatGraph(object):
    ...
    def get_unbranching_paths(self):
        ...
        for edge in self.edges.values():
            ...
            if not edge.self_complement:
                ...
                while (
                    not cur_node.is_bifurcation()
                    and len(cur_node.in_edges) > 0
                    and cur_node.in_edges[0] not in visited_edges
                    and not cur_node.in_edges[0].self_complement
                ):
                ...
                while (
                    not cur_node.is_bifurcation()
                    and len(cur_node.out_edges) > 0
                    and cur_node.out_edges[0] not in visited_edges
                    and not cur_node.out_edges[0].self_complement
                ):
            ...
            for edge in self.edges.values():
                f.write(
                    "Edge\t{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n".format(
                        _to_unsigned_id(edge.edge_id),
                        node_ids[edge.node_left],
                        node_ids[edge.node_right],
                        int(edge.repetitive),
                        int(edge.self_complement),
                        int(edge.resolved),
            ...
            for edge in self.edges.values():
                f.write(
                    '{0} -> {1} [label = "{2}", color = "{3}"]\n'.format(
                        ...
                        node_ids[edge.node_right],
                        edge.edge_id,
                        "red" if edge.repetitive else "black",
                    )
                )
            f.write("}")
    ...
    def separate_path(self, graph_path, new_seq_id, new_seq_seq):
        ...
        def separate_one(edges_path, new_edge_id, new_edge_seq):
            ...
            path_coverage = (
                edges_path[0].mean_coverage + edges_path[-1].mean_coverage
            ) // 2
