...
try:
    ...
    sys.exit(
        "setuptools package not found. " "Please use 'pip install setuptools' first"
    )
...
class MakeInstall(SetuptoolsInstall):
    def run(self):
        ...
        if os.path.isdir(self.install_scripts) and not os.access(
            self.install_scripts, os.W_OK
        ):
            sys.exit(
                "Error: no write permission for "
        ...
        bin_files = ["cflye-modules", "cflye-minimap2", "cflye-samtools"]
        for file in bin_files:
            if not os.path.isfile(os.path.join(build_dir, file)):
                sys.exit("Error: binary not found: " + file)
            shutil.copy2(os.path.join(build_dir, file), os.path.join(install_dir, file))
...
setup(
    ...
    packages=[
        ...
        "cflye/utils",
        "cflye/repeat_graph",
        "cflye/short_plasmids",
        "cflye/trestle",
        "cflye/tests",
    ],
    package_data={"cflye": ["config/bin_cfg/*", "tests/data/*"]},
    entry_points={"console_scripts": ["cflye = cflye.main:main"]},
    cmdclass={"build": MakeBuild, "install": MakeInstall},
)
